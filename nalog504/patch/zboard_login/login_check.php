...
// 회원 로그인 체크
	$result = mysqli_query($connect,"select * from $member_table where user_id='$user_id' and password=password('$password')") or error(mysqli_error($connect));
	$member_data = mysqli_fetch_array($result);
// 회원로그인이 성공하였을 경우 세션을 생성하고 페이지를 이동함
	if($member_data['no']) {
//n@log 쿠키생성
		setcookie("na3_member","{$_POST['user_id']}",time()+30*24*3600,"/");
		if($auto_login) {

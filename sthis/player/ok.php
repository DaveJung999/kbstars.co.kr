...
switch(isset($_REQUEST['mode']) ? $_REQUEST['mode'] : ''){
	...
	case 'modify':
		...
		else $goto = 'read.php?'	. href_qs('uid=' . (isset($_REQUEST['uid']) ? $_REQUEST['uid'] : ''), $qs_basic);
		back('',$goto);
		break;
	case 'delete':
		delete_ok($dbinfo);
...
/*if( $_REQUEST['mode'] and eregi('^[a-z0-9\-\_]+$',$_REQUEST['mode'])
		...
		// 해당 필드 데이터값 확정
			switch($value){
				...
				case 'docu_type' : // html값이 아니면 text로
					if(!isset($_POST['docu_type']) || !$_POST['docu_type']) $_POST['docu_type']=$dbinfo['default_docu_type'];
				...
				case 'num' :
					...
					break;
				case 'bid' :
					$qs['bid']	= isset($_SESSION['seUid']) ? $_SESSION['seUid'] : 0;
					break;
				case 'userid' :
			...
			// sql_set 만듦
			if(isset($qs[$value])){
				if($value == 'passwd') $sql_set .= ", passwd = PASSWORD('".db_escape($qs[$value]) . "') ";
...
function modify_ok(&$dbinfo,$qs){
	...
	// 해당 게시물 읽어오기
	$sql = "SELECT *, PASSWORD('".db_escape(isset($_POST['passwd']) ? $_POST['passwd'] : '') . "') as pass FROM {$dbinfo['table']} WHERE uid='".db_escape($qs['uid']) . "' and $sql_where LIMIT 1";
	$list=db_arrayone($sql) or back('수정할 게시물이 없습니다 . 확인 바랍니다.');
	
	// 수정 권한 체크
	if(!function_exists('privAuth') || !privAuth($dbinfo,'priv_modify') ){
		if(isset($list['bid']) && $list['bid']>0){
			if( $list['bid'] != $_SESSION['seUid'] || (isset($dbinfo['priv_modify']) && 'nobid' == substr($dbinfo['priv_modify'],0,5)) )
				back('수정하실 권한이 없습니다.');
		} else {
			if(isset($list['passwd']) && $list['passwd'] != $list['pass']) back('정확한 비밀번호를 입력하여 주십시오');
		}
	} // end if
...
*/
				...
				case 'docu_type' : // html값이 아니면 text로
					...
					break;
				case 'userid' :
					if(isset($list['bid']) && $list['bid'] == $_SESSION['seUid']) { // 관리자권한으로 수정했으면 변경불가
						switch($dbinfo['enable_userid']){
							case 'name'		: $qs['userid'] = $_SESSION['seName']; break;
							case 'nickname'	: $qs['userid'] = $_SESSION['seNickname']; break;
							default			: $qs['userid'] = $_SESSION['seUserid']; break;
				...
				case 'email' :
					if(isset($_POST['email'])) $qs['email']	= check_email($_POST['email']);
					elseif(isset($list['bid']) && $list['bid'] == $_SESSION['seUid']) // 관리자권한으로 수정했으면 변경불가
...
function delete_ok(&$dbinfo){
	...
	// 해당 게시물 읽어오기
	$sql = "SELECT *, PASSWORD('".db_escape($qs['passwd']) . "') as pass FROM {$dbinfo['table']} WHERE uid='".db_escape($qs['uid']) . "' and $sql_where LIMIT 1";
	...
	if(!function_exists('privAuth') || !privAuth($dbinfo,'priv_delete')) {// 게시판 전체 삭제 권한을 가졌다면
		if( (isset($dbinfo['priv_delete']) && 'nobid' == substr($dbinfo['priv_delete'],0,5)) )
			back('삭제하실 수 없습니다.');
	...
	// 삭제
	db_query("DELETE FROM {$dbinfo['table']} WHERE uid='".db_escape($qs['uid']) . "' and  $sql_where ");
	return true;
} // end func delete_ok()

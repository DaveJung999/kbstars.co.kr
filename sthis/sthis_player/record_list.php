...
if(isset($dbinfo['enable_cate']) && $dbinfo['enable_cate'] == 'Y'){
	...
	$cateinfo=boardCateInfo($dbinfo, $table_cate, $_GET['cateuid'] ?? null, 'Y', 1,1,$tmp_itemcount,"(종합)");
...
if(isset($dbinfo['enable_getinfo']) && $dbinfo['enable_getinfo'] == 'Y'){
	...
	if( isset($_GET['html_headtpl']) && preg_match("/^[_a-z0-9]+$/",$_GET['html_headtpl'])
...
if(isset($dbinfo['table_name']) && isset($dbinfo['db']) && $dbinfo['table_name'] != $dbinfo['db']) {
	$sql_where=" db='{$dbinfo['db']}' "; // $sql_where 사용 시작
}
...
$sql_orderby = $dbinfo['orderby'] ?? "	num DESC, re ";
...
$count=board2Count($count['total'],$page,$dbinfo['pern'] ?? null,$dbinfo['page_pern'] ?? null); // 각종 카운트 구하기
$count['today']=db_resultone("SELECT count(*) FROM {$table} WHERE (rdate > unix_timestamp(curdate())) and $sql_where " , 0, "count(*)");

//-------------spemin
if(!isset($dbinfo['pern'])) $dbinfo['pern']		= $count['total'];
if(!isset($dbinfo['row_pern'])) $dbinfo['row_pern']		= $count['total'];
//-------------------
...
// Limit로 필요한 게시물만 읽음.
$limitno	= $_GET['limitno'] ?? $count['firstno'];
$limitrows	= $_GET['limitrows'] ?? $count['pern'];
...
if(!$total=db_count($rs_list)) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			if($list['rdate']>(time()-3600*24)) $list['enable_new']="<img src='/images/icon_new.gif' width='30' height='15' border='0'>";
			else $list['enable_new'] = ""; // 추가

			// 업로드파일 처리
			if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] != 'N' and isset($list['upfiles'])){
			...
			// 템플릿 YESRESULT 값들 입력
			if(isset($list['uid']) && $list['uid'] == 25) $list['color'] = "#E7D3FD";
...
// - 사이트 템플릿 읽어오기
if(isset($dbinfo['html_headpattern']) && preg_match("^(ht|h|t)$",$dbinfo['html_headpattern'])){
	$HEADER['header'] = 2;
	if( isset($dbinfo['html_headtpl']) && $dbinfo['html_headtpl'] != "" && is_file("{$_SERVER['DOCUMENT_ROOT']}/stpl/basic/index_{$dbinfo['html_headtpl']}.php") )

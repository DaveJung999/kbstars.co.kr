...
//=======================================================
	...
	if($mode === 'modify'){
		...
		if (isset($list['data2'])) {
			$list["data2_{$list['data2']}"] = " checked";
		}
	...
	// URL Link...
	$href['list']	= ($thisUrl ?? '') .'listall.php?'.$qs_basic;
//=======================================================
...
if (isset($list['data2'])) {
	$tpl->tie_var("list.data2_{$list['data2']}" ,($list["data2_{$list['data2']}"] ?? ''));
}
 ...
 * @param string $table
 * @param array|string $skip_fields
 * @return array|false
 */
function userGetAppendFields(string $table, $skip_fields='') {
	global $mysqli;

	if(!is_array($skip_fields)) {
		$skip_fields = array();
	}
	
	//davej.................. . 2007-03-28 임시..............
	// 원본 코드의 하드코딩된 값을 유지합니다.
	$database = "savers_secret";
	$table = "new21_slist_totalgame_result";

	$fieldlist = array();
	$safe_table = $mysqli->real_escape_string($table);
	$safe_database = $mysqli->real_escape_string($database);
	$result = $mysqli->query("SHOW COLUMNS FROM `{$safe_database}`.`{$safe_table}`");
	if (!$result) {
		return false;
	}

...
//=======================================================
	...
	else $qs_basic = "mode=&limitno=&limitrows=&html_headpattern=&html_headtpl=&skin=";
	$qs_basic			= href_qs($qs_basic); // 해당값 초기화

	$table_dbinfo	= $SITE['th'] . "boardinfo";
	$table_readlog	= $SITE['th'] . "boardreadlog"; // 유저별 읽은 유무 로그
	...
	// 인증 체크(자기 글이면 무조건 보기)
	if(!boardAuth($dbinfo, "priv_read",1)){
		if($list['bid']){
			if($list['bid']<>$_SESSION['seUid']){
				...
				if(strlen($list['re']) == 0){
					...
					$sql_where_boardauth = " $sql_where and num='{$list['num']}' and (re='' ";
					for($i=0;$i<strlen($list['re'])-1;$i++){
						$sql_where_boardauth .= " or re='" . substr($list['re'],0,$i+1) ."' ";
					}
					$sql_where_boardauth .= ") and bid='{$_SESSION['seUid']}' ";
					$sql = "select * from {$table} where {$sql_where_boardauth}";
					if(!db_arrayone($sql))
						back("이용이 제한되었습니다.(레벨부족)");
				} // end if..else..
			} // end if
	...
	// 비공개글 제외시킴
	if($dbinfo['enable_level'] == 'Y' and !boardAuth($list, "priv_level",1)){
	...
	//=====
	// 조회수 증가
	if(isset($_SERVER['REMOTE_ADDR']) && (!isset($_SESSION['seUid']) || $list['bid'] !== $_SESSION['seUid'])){
		$sql = "UPDATE LOW_PRIORITY {$table} SET hit=hit +1, hitip='{$_SERVER['REMOTE_ADDR']}' WHERE uid='{$_GET['uid']}' and hitip<>'{$_SERVER['REMOTE_ADDR']}' and (bid<>'{$_SESSION['seUid']}' or	1>'{$_SESSION['seUid']}') LIMIT 1";
		db_query($sql);
	}
	...
	if($dbinfo['enable_readlist'] == 'Y' and $dbinfo['row_pern']<2 ){
		...
		while($readlist=db_array($re_readlist)){
			...
			if(isset($sc_string)){
				if(isset($sc_column)){
					if($sc_column == "title")
						$readlist['cut_title'] = preg_replace("/({$sc_string})/i", "<font color=darkred>\\1</font>",	$readlist['cut_title']);
					else
						$readlist[$sc_column]	= preg_replace("/({$sc_string})/i", "<font color='darkred'>\\1</font>", $readlist[$sc_column]);
				} else {
					$readlist['userid']	= preg_replace("/({$sc_string})/i", "<font color=darkred>\\1</font>", $readlist['userid']);
					$readlist['cut_title']= preg_replace("/({$sc_string})/i", "<font color=darkred>\\1</font>",	$readlist['cut_title']);
				}
			}
			...
			// 업로드파일 처리
			if($dbinfo['enable_upload'] != 'N' and isset($readlist['upfiles']) and $readlist['upfiles']){
...
//===============
	if($dbinfo['enable_memo'] == 'Y'){
		$sql_where_memo="";
		// 메모 테이블 구함
		if($dbinfo['table_name'] != $dbinfo['db']) $sql_where_memo=" db='{$dbinfo['db']}' and "; // $sql_where 사용 시작
		if($dbinfo['enable_type'] == "Y"){
			$table_memo		= $table;
			$sql_where_memo.= " type='memo' ";
		} else {
		...
		switch($dbinfo['enable_userid']){
			case 'name'		: $memouserid = $_SESSION['seName']; break;
			case 'nickname'	: $memouserid = $_SESSION['seNickname']; break;
			default			: $memouserid= $_SESSION['seUserid']; break;
		}
...
// 블럭 : 카테고리(상위, 동일, 서브) 생성
	if($dbinfo['enable_cate'] == 'Y'){
		...
		if(isset($i) && $i == 0) $tpl->drop_var('blockloop');
...
// 블럭 : 글쓰기
	if(boardAuth($dbinfo, "priv_write")) $tpl->process('WRITE','write');

// 블럭 : 글답변
	if(boardAuth($dbinfo, "priv_reply")) $tpl->process('REPLY','reply');

// 블럭 : 글수정,삭제
	if(boardAuth($dbinfo, "priv_delete") or (isset($list['bid']) && isset($_SESSION['seUid']) && $list['bid'] == $_SESSION['seUid']) or (isset($list['bid']) && $list['bid'] == 0)){
...
// 마무리
	...
	switch($dbinfo['html_headpattern']){
		case "ht":
			// 전체 홈페이지 템플릿 읽어오기
			$HEADER['header'] == 2;
			if( isset($dbinfo['html_headtpl']) and $dbinfo['html_headtpl'] != "" and is_file("{$_SERVER['DOCUMENT_ROOT']}/stpl/basic/index_{$dbinfo['html_headtpl']}.php") )

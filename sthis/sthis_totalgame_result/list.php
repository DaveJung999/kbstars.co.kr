...
//=======================================================
	...
	$qs_basic = "db={$_GET['db']}".					//table 이름
				"&mode=".					// mode값은 list.php에서는 당연히 빈값
				"&cateuid={$_GET['cateuid']}".		//cateuid
				"&pern={$_GET['pern']}" .	// 페이지당 표시될 게시물 수
				"&sc_column={$sc_column}".	//search column
				"&sc_string=" . urlencode(stripslashes($sc_string)) . //search string
				"&page={$page}";				//현재 페이지

	if($_GET['skin'] == ""){	include_once("./dbinfo.php"); // $dbinfo, $table 값 정의
	} else {
		$table = $_GET['db'];
	...
	// 인증 체크
	if(!boardAuth($dbinfo, "priv_list",1)) back("이용이 제한되었습니다.(레벨부족)");
	...
	if($dbinfo['enable_cate'] == 'Y'){
		...
		$cateinfo=board2CateInfo($dbinfo, $_GET['cateuid'], $sw_catelist, "(종합)");
	...
	if($dbinfo['enable_getinfo'] == 'Y'){
		if($_GET['cut_length'])	$dbinfo['cut_length']	= $_GET['cut_length'];
		if($_GET['pern'])			$dbinfo['pern']		= $_GET['pern'];

		// skin관련
		if($_GET['html_headpattern'])	$dbinfo['html_headpattern'] = $_GET['html_headpattern'];
		if( $_GET['html_headtpl'] and preg_match("/^[_a-z0-9]+$/",$_GET['html_headtpl'])
			and is_file("{$_SERVER['DOCUMENT_ROOT']}/stpl/basic/index_{$_GET['html_headtpl']}.php") )
			$dbinfo['html_headtpl'] = $_GET['html_headtpl'];
		if( $_GET['skin'] and preg_match("/^[_a-z0-9]+$/",$_GET['skin'])
	...
	// 게시물 일부만 본다면
	if($_GET['limitrows']) $dbinfo['pern'] = $count['total'];
...
if($dbinfo['enable_writeinfo'] == 'Y' and $dbinfo['enable_type'] == 'Y' and $dbinfo['row_pern']<2 and strlen($_GET['sc_string']) == 0 and $_GET['limitrows']<1){
	// 공지글은 검색시, iframe으로 일부만 볼때, 그리고 한줄에 여러줄 출력할때는 안보인다.
	if(strlen($_GET['sc_string']) == 0 and strlen($_GET['skin']) == 0 and $dbinfo['row_pern']<2 ){
		$sql_where_info = " db='{$dbinfo['db']}' and type='info' ";
		// 공지도 해당 카테고리만
		if(is_array($cateinfo['subcate_uid']) and sizeof($cateinfo['subcate_uid'])>0 ) $sql_where_info = $sql_where_info ? $sql_where_info	. " and ( cateuid in ( " . implode(",",$cateinfo['subcate_uid']) . ") ) " : " ( cateuid in ( " . implode(",",$cateinfo['subcate_uid']) . ") ) ";
		if(!$sql_where_info) $sql_where_info = " 1 ";
		$sql = "SELECT * FROM {$table} WHERE {$sql_where_info} ORDER BY data1 desc, data2 desc";
...
$sql = "SELECT * FROM {$table} WHERE $sql_where ORDER BY	data1 desc, uid desc LIMIT {$limitno},{$limitrows}";
...
if(!$total=db_count($rs_list)) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
			...
			if($_GET['sc_string']){
				if($_GET['sc_column']){
					if($_GET['sc_column'] == "title")
						$list['cut_title'] = preg_replace("/({$_GET['sc_string']})/i", "<font color=darkred>\\1</font>",	$list['cut_title']);
					else
						$list[$_GET['sc_column']]	= preg_replace("/({$_GET['sc_string']})/i", "<font color='darkred'>\\1</font>", $list[$_GET['sc_column']]);
				} else {
					$list['userid']	= preg_replace("/({$_GET['sc_string']})/i", "<font color=darkred>\\1</font>", $list['userid']);
					$list['cut_title']= preg_replace("/({$_GET['sc_string']})/i", "<font color=darkred>\\1</font>",	$list['cut_title']);
				}
			}
		...
		$list['tr_season'] = $trs1."<br>".$trs2;
			...
			if(boardAuth($dbinfo, "priv_write")) $tpl->process('GO','go');
...
if(!$_GET['limitrows']) { // 게시물 일부 보기에서는 카테고리, 블럭이 필요 없을 것임
	// 블럭 : 카테고리(상위, 동일, 서브) 생성
	if($dbinfo['enable_cate'] == 'Y'){

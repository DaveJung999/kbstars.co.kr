...
// 25/08/11 AI		PHP 7.0 이상 버전 호환성 수정
// 25/08/12 AI		write_ok, reply_ok, modify_ok 함수의 SQL 쿼리 오류 수정
//=======================================================
...
switch(isset($_REQUEST['mode']) ? $_REQUEST['mode'] : ''){
	...
	case 'modify':
		...
		go_url(isset($_REQUEST['goto']) ? $_REQUEST['goto'] : "{$thisUrl}/list.php?" . href_qs("uid={$uid}",$qs_basic));
		break;
	case 'delete':
		$goto = isset($_REQUEST['goto']) ? $_REQUEST['goto'] : "{$thisUrl}/list.php?" . href_qs("",$qs_basic);
...
// VOTE
	case 'vote' :
		$vote=vote_ok();
		back("현재 {$vote}점입니다.","" . (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : ''));
		break;	
// 메모
	case 'memowrite':
		memoWrite_ok();
		go_url(isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '');
		break;
	case 'memodelete':
		memodelete_ok();
		go_url(isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '');
...
function write_ok($table, $qs){
	...
	if(!function_exists('privAuth') || !privAuth($dbinfo, "priv_write")) back("이용이 제한되었습니다(레벨부족) . 확인바랍니다.");
	...
	$qs['priv_level']=(int)(isset($qs['priv_level']) ? $qs['priv_level'] : 0);
	...
	if(isset($_SESSION['seUid'])){
		...
		$qs['email']	= check_email(isset($qs['email']) ? $qs['email'] : '');
	}
	$qs['ip']		= remote_addr();
	// - num의 최대값 구함
	$sql_where = '';
			...
			foreach($_FILES as $key =>  $value){
				if(isset($_FILES[$key]['name']) && $_FILES[$key]['name']) { // 파일이 업로드 되었다면
					if( $dbinfo['enable_upload'] == 'image'
						AND (!isset($_FILES[$key]['tmp_name']) || !is_array(getimagesize($_FILES[$key]['tmp_name']))) )
						continue;
	...
	// 추가되어 있는 테이블 필드 포함
	$sql_set = '';
	$skip_fields = array('uid', 'bid', 'userid', 'email', 'passwd', 'db', 'cateuid', 'num', 're', 'title', 'content', 'upfiles', 'upfiles_totalsize', 'docu_type', 'type', 'priv_level', 'ip', 'hit', 'hitip', 'hitdownload', 'vote', 'voteip' ,	'rdate', 'tr_season', 'tr_ranking', 'tr_game', 'tr_win', 'tr_loss', 'tr_score', 'tr_2p1', 'tr_2p2', 'tr_3p1', 'tr_3p2', 'tr_free1', 'tr_free2', 'tr_re', 'tr_as', 'tr_st', 'tr_blk', 'tr_to', 'tr_po'); // 여기에도 추가 필드들을 추가
	...
	$qs['rdate'] = strtotime(isset($qs['rdate']) ? $qs['rdate'] : 'now');
	
	$qs['tg_year'] = date("Y",$qs['rdate']);
	$qs['tg_month'] = date("m",$qs['rdate']);
	$qs['tg_day'] = date("d",$qs['rdate']);
	$qs['tr_season'] = substr(isset($qs['data1']) ? $qs['data1'] : '', -2).(isset($qs['data2']) ? $qs['data2'] : '');//davej...........시즌순 정렬때문에....

	// sql문 완성
	if(isset($dbinfo['enable_type']) && $dbinfo['enable_type'] == 'Y' && isset($qs['writeinfo']) && $qs['writeinfo'] == "info") $sql_set	.= ", type='info' ";// $sql_set 시작
	$sql="INSERT
			INTO
				{$table}
			SET
				`bid`		='" . (isset($qs['bid']) ? db_escape($qs['bid']) : '') . "',
				`userid`	='" . (isset($qs['userid']) ? db_escape($qs['userid']) : '') . "',
				`title`		='" . (isset($qs['title']) ? db_escape($qs['title']) : '') . "',
				`data1`		='" . (isset($qs['data1']) ? db_escape($qs['data1']) : '') . "',
				`data2`		='" . (isset($qs['data2']) ? db_escape($qs['data2']) : '') . "',
				`tr_season`	='" . (isset($qs['tr_season']) ? db_escape($qs['tr_season']) : '') . "',
				`tr_ranking`	='" . (isset($qs['tr_ranking']) ? db_escape($qs['tr_ranking']) : '') . "',
				`tr_game`	='" . (isset($qs['tr_game']) ? db_escape($qs['tr_game']) : '') . "',
				`tr_win`	='" . (isset($qs['tr_win']) ? db_escape($qs['tr_win']) : '') . "',
				`tr_loss`	='" . (isset($qs['tr_loss']) ? db_escape($qs['tr_loss']) : '') . "',
				`tr_score`	='" . (isset($qs['tr_score']) ? db_escape($qs['tr_score']) : '') . "',
				`tr_2p1`	='" . (isset($qs['tr_2p1']) ? db_escape($qs['tr_2p1']) : '') . "',
				`tr_2p2`	='" . (isset($qs['tr_2p2']) ? db_escape($qs['tr_2p2']) : '') . "',
				`tr_3p1`	='" . (isset($qs['tr_3p1']) ? db_escape($qs['tr_3p1']) : '') . "',
				`tr_3p2`	='" . (isset($qs['tr_3p2']) ? db_escape($qs['tr_3p2']) : '') . "',
				`tr_free1`	='" . (isset($qs['tr_free1']) ? db_escape($qs['tr_free1']) : '') . "',
				`tr_free2`	='" . (isset($qs['tr_free2']) ? db_escape($qs['tr_free2']) : '') . "',
				`tr_re`		='" . (isset($qs['tr_re']) ? db_escape($qs['tr_re']) : '') . "',
				`tr_as`		='" . (isset($qs['tr_as']) ? db_escape($qs['tr_as']) : '') . "',
				`tr_st`		='" . (isset($qs['tr_st']) ? db_escape($qs['tr_st']) : '') . "',
				`tr_blk`	='" . (isset($qs['tr_blk']) ? db_escape($qs['tr_blk']) : '') . "',
				`tr_to`		='" . (isset($qs['tr_to']) ? db_escape($qs['tr_to']) : '') . "',
				`tr_po`		='" . (isset($qs['tr_po']) ? db_escape($qs['tr_po']) : '') . "'
				{$sql_set_file}
				{$sql_set}
		";
	db_query($sql);
	...
	if(isset($dbinfo['enable_adm_mail']) && $dbinfo['enable_adm_mail'] == 'Y'){
		...
		$mail->name		= "게시판 자동메일";
		$mail->to		= (isset($list['email']) ? $list['email'] : '');
		$mail->subject	= "[답변] " . (isset($qs['title']) ? $qs['title'] : '');
...
function reply_ok($table,$qs){
	...
	if(!function_exists('privAuth') || !privAuth($dbinfo, "priv_reply")) back("이용이 제한되었습니다(레벨부족) . 확인바랍니다.");
	...
	// 정상적인 질문에 대한 답변인지 체크
	$sql = "SELECT * FROM {$table} WHERE uid='".db_escape($qs['uid']) . "' and PASSWORD(rdate)='".db_escape($qs['private_key']) . "'";
	...
	$sql="INSERT
			INTO
				{$table}
			SET
				db		='" . (isset($qs['db']) ? db_escape($qs['db']) : '') . "',
				num		='" . (isset($qs['num']) ? db_escape($qs['num']) : '') . "',
				re		='" . (isset($qs['re']) ? db_escape($qs['re']) : '') . "',
				bid		='" . (isset($qs['bid']) ? db_escape($qs['bid']) : '') . "',
				userid	='" . (isset($qs['userid']) ? db_escape($qs['userid']) : '') . "',
				passwd	=PASSWORD('" . (isset($qs['passwd']) ? db_escape($qs['passwd']) : '') . "'),
				email	='" . (isset($qs['email']) ? db_escape($qs['email']) : '') . "',
				title	='" . (isset($qs['title']) ? db_escape($qs['title']) : '') . "',
				content	='" . (isset($qs['content']) ? db_escape($qs['content']) : '') . "',
				docu_type='" . (isset($qs['docu_type']) ? db_escape($qs['docu_type']) : '') . "',
				rdate	=UNIX_TIMESTAMP(),
				ip		='" . (isset($qs['ip']) ? db_escape($qs['ip']) : '') . "',
				cateuid ='" . (isset($qs['cateuid']) ? db_escape($qs['cateuid']) : '') . "',
				priv_level	='" . (isset($qs['priv_level']) ? db_escape($qs['priv_level']) : '') . "'
				{$sql_set_file}
				{$sql_set}
		";
	db_query($sql);
	...
	// E-Mail 전송
	if( (isset($dbinfo['enable_adm_mail']) && $dbinfo['enable_adm_mail'] == 'Y') || (isset($dbinfo['enable_rec_mail']) && $dbinfo['enable_rec_mail'] == 'Y') ){
		$mail = new mime_mail;
		$mailfrom = '';
		
		if(isset($dbinfo['enable_adm_mail']) && $dbinfo['enable_adm_mail'] == 'Y'){
			$sql = "select email from {$table_logon} where uid='" . (isset($dbinfo['bid']) ? db_escape($dbinfo['bid']) : '') . "'";
			$email_result = db_resultone($sql,0,"email");
			if($email_result && ($dbinfo['email'] = check_email($email_result)))
...
function modify_ok($table,$qs,$field){
	...
	$sql = '';
	if(function_exists('privAuth') && privAuth($dbinfo,"priv_delete")) { // 게시판 전체 삭제 권한을 가졌다면 수정 권한 무조건 부여
		$sql = "SELECT * FROM {$table} WHERE uid='".db_escape($qs['uid']) . "'";
	} elseif(isset($_SESSION['seUid'])) { // 회원의 글이라면,
		$sql = "SELECT * FROM {$table} WHERE uid='".db_escape($qs['uid']) . "' and bid='".db_escape($_SESSION['seUid']) . "'";
	} else { // 비회원의 글이라면 (비회원의 글에 패스워드가 없을 경우 누구든지 수정 가능, 실수로 안 입력했을 경우 수정가능하게)
		$sql = "SELECT * FROM {$table} WHERE uid='".db_escape($qs['uid']) . "' and passwd=PASSWORD('".db_escape(isset($qs['passwd']) ? $qs['passwd'] : '') . "')";
	} // end if
	if(!$list=db_arrayone($sql)) back("게시물이 없거나 수정할 권한이 없습니다");
		
	// 값 추가
	if(isset($list['bid']) && $list['bid'] == (isset($_SESSION['seUid']) ? $_SESSION['seUid'] : '')){
		switch($dbinfo['enable_userid']){
			case 'name'		: $qs['userid'] = $_SESSION['seName']; break;
			case 'nickname'	: $qs['userid'] = $_SESSION['seNickname']; break;
			default			: $qs['userid'] = $_SESSION['seUserid']; break;
		...
		$qs['userid']	= isset($list['userid']) ? $list['userid'] : '';
		$qs['email']	= (isset($qs['email']) && $qs['email']) ? check_email($qs['email']): (isset($list['email']) ? $list['email'] : ''); // email값이 넘어오면 수정하고 아니면 그대로 유지
	}
	$qs['ip']		= remote_addr();
	$qs['cateuid']= (isset($qs['catelist']) && isset($list['re']) && strlen($list['re']) == 0 ) ? $qs['catelist'] : (isset($list['cateuid']) ? $list['cateuid'] : ''); // 답변이 아닌 경우에만 카테고리 수정 가능
	...
	if(isset($qs['cateuid']) && isset($list['cateuid']) && $qs['cateuid'] <> $list['cateuid'] ){
		db_query("update {$table} set cateuid='".db_escape($qs['cateuid']) . "' where db='" . (isset($list['db']) ? db_escape($list['db']) : '') . "' and type='" . (isset($list['type']) ? db_escape($list['type']) : '') . "' and num='" . (isset($list['num']) ? db_escape($list['num']) : '') . "'");
	} // end if
...
function delete_ok($table,$field,$goto){
	...
	// SQL문 where절 정리
	$sql_where = '';
	if(isset($dbinfo['table_name']) && isset($dbinfo['db']) && $dbinfo['table_name'] != $dbinfo['db']) $sql_where=" db='{$table}' "; // $sql_where 사용 시작
	if(isset($dbinfo['enable_type']) && $dbinfo['enable_type'] == 'Y') $sql_where = $sql_where ? $sql_where	. " and (type='docu' or type='info') " : " (type='docu' or type='info') ";
	
	if (!$sql_where) $sql_where = '1';

	// 삭제 권한 체크와 해당 게시물 읽어오기
	$sql = "SELECT *, PASSWORD('".db_escape(isset($qs['passwd']) ? $qs['passwd'] : '') . "') as pass FROM {$table} WHERE uid='".db_escape($qs['uid']) . "' and  $sql_where ";
	
	$list = db_arrayone($sql) or back("이미 삭제되었거나 잘못된 요청입니다");
	if(!function_exists('privAuth') || !privAuth($dbinfo,"priv_delete")) {// 게시판 전체 삭제 권한을 가졌다면
		if( (isset($list['bid']) && $list['bid'] == 0) and (isset($list['passwd']) && $list['passwd'] != (isset($list['pass']) ? $list['pass'] : ''))){
			if(isset($_SERVER['QUERY_STRING']))
				back("비밀번호를 입력하여 주십시오","{$thisUrl}/delete.php?{$_SERVER['QUERY_STRING']}");
			else back("비밀번호를 정확히 입력하십시오");
		} elseif (isset($list['bid']) && $list['bid']>0 and $list['bid'] != (isset($_SESSION['seUid']) ? $_SESSION['seUid'] : '')) back("올린이가 아님니다.");
	}
	...
	if(isset($list['upfiles']) && $list['upfiles']){
		...
		foreach($upfiles as $key =>  $value){
			if(isset($value['name']) && $value['name']){
				if(isset($list['bid']) && is_file($dbinfo['upload_dir'] . "/{$list['bid']}/" . $value['name']) ){
	...
	// 답변글과 파일도 삭제
	$del_uploadfile = [];
	$rs_subre = db_query("SELECT * FROM {$table} WHERE $sql_where and num='" . (isset($list['num']) ? db_escape($list['num']) : '') . "' AND length(re) > length('" . (isset($list['re']) ? db_escape($list['re']) : '') . "') AND locate('" . (isset($list['re']) ? db_escape($list['re']) : '') . "',re) = 1");
	while($row=db_array($rs_subre)){
		if(isset($row['upfiles']) && $row['upfiles']){
			...
			if(!is_array($upfiles)) { // 시리얼화된 변수가 아닌 파일 명으로 되어 있을 경우
				...
				$upfiles['upfile']['size']=(int)(isset($row['upfiles_totalsize']) ? $row['upfiles_totalsize'] : 0);
			}
			foreach($upfiles as $key =>  $value){
				if(isset($value['name']) && $value['name']){
					if(isset($row['bid']) && is_file($dbinfo['upload_dir'] . "/{$row['bid']}/" . $value['name']) )
	...
	db_query("DELETE FROM {$table} WHERE $sql_where and num='" . (isset($list['num']) ? db_escape($list['num']) : '') . "' AND length(re) > length('" . (isset($list['re']) ? db_escape($list['re']) : '') . "') AND locate('" . (isset($list['re']) ? db_escape($list['re']) : '') . "',re) = 1");
	db_query("DELETE FROM {$table} where $sql_where and uid='" . (isset($list['uid']) ? db_escape($list['uid']) : '') . "'");
...
function vote_ok(){
	...
	// 점수 한계선 설정
	$qs['vote'] = (int)(isset($qs['vote']) ? $qs['vote'] : 0);
	if($qs['vote']>5) $qs['vote']=5;
	if($qs['vote']<-5) $qs['vote']=-5;

	// 조회수 증가
	db_query("UPDATE {$table} SET
					vote	=vote +".db_escape($qs['vote']) . ",
					voteip	='" . (isset($_SERVER['REMOTE_ADDR']) ? db_escape($_SERVER['REMOTE_ADDR']) : '') . "'
				WHERE
					uid='".db_escape($qs['uid']) . "'
				AND
					voteip<>'" . (isset($_SERVER['REMOTE_ADDR']) ? db_escape($_SERVER['REMOTE_ADDR']) : '') . "'
	...
	if(function_exists('db_count') && db_count())
		return (isset($info['vote']) ? $info['vote'] : 0) + $qs['vote'];
	else
		back("이미 참여하셨습니다.");
...
function memoWrite_ok(){
	...
	if(isset($dbinfo['enable_memo']) && $dbinfo['enable_memo'] == 'Y'){
		...
		if(isset($dbinfo['table_name']) && isset($dbinfo['db']) && $dbinfo['table_name'] != $dbinfo['db']){
			$sql_set_memo.=" , db='" . (isset($qs['db']) ? db_escape($qs['db']) : '') . "' ";
		} // end if
	...
	$sql="INSERT
			...
			SET
				bid		='" . (isset($qs['bid']) ? db_escape($qs['bid']) : '') . "',
				userid	='" . (isset($qs['userid']) ? db_escape($qs['userid']) : '') . "',
				num		='" . (isset($qs['uid']) ? db_escape($qs['uid']) : '') . "',
				title	='" . (isset($qs['title']) ? db_escape($qs['title']) : '') . "',
				rdate	=UNIX_TIMESTAMP(),
				ip		='" . (isset($qs['ip']) ? db_escape($qs['ip']) : '') . "'
				{$sql_set_memo}
...
function memodelete_ok(){
	...
	$table_memo = '';
	if(isset($dbinfo['enable_memo']) && $dbinfo['enable_memo'] == 'Y'){
		if(isset($dbinfo['table_name']) && isset($qs['db']) && $dbinfo['table_name'] != $qs['db']){
			$sql_where_memo.=" and db='".db_escape($qs['db']) . "' "; // $sql_set_memo 사용 시작
		} // end if
		if(isset($dbinfo['enable_type']) && $dbinfo['enable_type'] == "Y"){
	...
	// 삭제 권한 체크와 해당 게시물 읽어오기
	$sql = "SELECT *, PASSWORD('".db_escape(isset($qs['passwd']) ? $qs['passwd'] : '') . "') as pass FROM {$table_memo} WHERE uid='".db_escape($qs['memouid']) . "' and {$sql_where_memo}";
	$list = db_arrayone($sql) or back("이미 삭제되었거나 잘못된 요청입니다");
	if(!function_exists('privAuth') || !privAuth($dbinfo,"priv_delete")) {// 게시판 전체 삭제 권한을 가졌다면
		if( (isset($list['bid']) && $list['bid'] == 0) and (isset($list['passwd']) && $list['passwd'] != (isset($list['pass']) ? $list['pass'] : ''))){
	...
	db_query("DELETE FROM {$table_memo} WHERE uid='".db_escape($qs['memouid']) . "'");

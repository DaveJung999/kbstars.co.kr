...
if (isset($_POST['passwd'])){
	...
	if ($encoded !== false) { // base64_decode V7+ returns false on failure
	...
	case 'write':
		...
		if ( isset($dbinfo['db']) && $dbinfo['db'] == '2010kbintern' ){
			...
			if (isset($list_cateuid['uid'])){
				$sql_kbintern = "SELECT count(uid) as cnt FROM {$dbinfo['table']} WHERE cateuid = '".db_escape($list_cateuid['uid'])."' ";
...
function reply_ok(&$dbinfo, $qs){
	...
	// $qs 추가, 체크후 값 가져오기
	$qs['uid']	= 'post,trim,notnull=' . urlencode('답변할 게시물의 고유넘버가 넘어오지 않았습니다.'); // 'trim' (오타 수정)
	$qs=check_value($qs);
			...
			switch($value){
				...
				case 'docu_type' : // html값이 아니면 text로
					if(!isset($_POST['docu_type']) || !$_POST['docu_type']){
						$_POST['docu_type'] = $dbinfo['default_docu_type'] ?? 'text';
					}
				...
				case 'userid' :
					...
					} elseif ((($dbinfo['priv_reply'] ?? 0) > 0) && !isset($_SESSION['seUid'])){
	...
	if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] != 'N' and isset($_FILES)){
		...
		if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] == 'Y'){
			$POSTFILE['upfile'] = $_FILES['upfile']; // upfile 하나만 업로드 (참조 & 제거)
		} else {
			$POSTFILE = $_FILES; // 모든 업로두 파일 (참조 & 제거)
		}
...
function write_ok(&$dbinfo, $qs){
	...
	// 같은글 등록 막기
	$user_id = $_POST['userid'] ?? ($_SESSION['seName'] ?? '');
		...
		foreach($fieldlist as $value){
			...
			switch($value){
				...
				case 'cateuid' : // catelist에서 선택한 값을
					...
					if(isset($_POST['catelist']) && ($list['cateuid'] ?? null) != ($qs['cateuid'] ?? null)){
						$sw_change_cateuid = 1;
					}
					break;
				case 'catetitle' :
				...
				case 'title' : //davej.........모든 태그 삭제
					...
					break;				
				case 'docu_type' : // html값이 아니면 text로
					if(!isset($_POST['docu_type']) || !$_POST['docu_type']) $_POST['docu_type'] = $dbinfo['default_docu_type'] ?? 'text';
					$_POST['docu_type'] = strtolower($_POST['docu_type']);
					if($_POST['docu_type'] != 'html'){
						$_POST['docu_type']='text';
					}
...
function delete_ok(&$dbinfo){
	...
	if(isset($list['num']) && $list['num']){
		...
		$sql_subre = "SELECT * FROM {$dbinfo['table']} WHERE num='{$num_safe}' AND length(re) > {$re_len_safe} AND locate('{$re_safe}',re) = 1";
		$rs_subre = db_query($sql_subre);
		
		if ($rs_subre) { // 쿼리 성공 확인
			while($row=db_array($rs_subre)){
				if(isset($row['upfiles']) && $row['upfiles']){
					$updir = $dbinfo['upload_dir'] . '/' . (int)($row['bid'] ?? 0);
...
function userGetAppendFields(string $table, array $skip_fields = [])
{
	...
	// 테이블 이름은 db_escape로 감싸고 백틱으로 묶어 안전하게 처리
...
function userReplyRe($table, $num, $re){
	...
	// [수정] $table 변수를 첫 번째 함수처럼 이스케이프 처리합니다.
	$safe_table = db_escape($table);
	$num_safe = db_escape($num);
	$re_safe = db_escape($re);
	
	// [수정] Tibero/Oracle 호환성을 위해 SQL 함수(right, locate, LIMIT)를 (SUBSTR, INSTR, ROWNUM)으로 변경합니다.
	// [수정] 'LAST_CHAR' 별칭(Alias)을 추가합니다.
	// [수정] $table 변수에 백틱(`)을 추가합니다. (userGetAppendFields 함수와 동일하게)
	
	/* 원본 MySQL 쿼리
	$sql = "SELECT re, right(re,1) FROM {$table} WHERE {$sql_where} and num='{$num_safe}' AND length(re)=length('{$re_safe}')+1 AND locate('{$re_safe}', re) = 1 ORDER BY re DESC LIMIT 1";
	*/

	// [수정된 쿼리] Tibero/Oracle 호환 쿼리 (ROWNUM 사용을 위해 서브쿼리 사용)
	$sql = "SELECT * FROM (
				SELECT
					re,
					SUBSTR(re, -1) AS LAST_CHAR
				FROM
					`{$safe_table}`
				WHERE
					{$sql_where}
					AND num='{$num_safe}'
					AND LENGTH(re)=LENGTH('{$re_safe}')+1
					AND INSTR(re, '{$re_safe}') = 1
				ORDER BY
					re DESC
			) WHERE ROWNUM = 1";
	...
	if ($row) {
		...
		// [수정] $row['right(re,1)'] 대신 안정적인 별칭 $row['LAST_CHAR']를 사용합니다.
		// (참고: Tibero/Oracle은 보통 별칭을 대문자로 반환합니다)

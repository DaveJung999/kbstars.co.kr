...
/*	if($_SERVER['REMOTE_ADDR'] == '61.35.254.195'){
	...
	if( isset($dbinfo['enable_adm_mail']) && $dbinfo['enable_adm_mail'] == 'Y' and (isset($_REQUEST['mode']) && ($_REQUEST['mode'] == 'write' or $_REQUEST['mode'] == 'reply')) ){
		$sql = "select email from {$table_logon} where uid='".db_escape($dbinfo['bid'] ?? '')."' limit 1";
...
switch($_REQUEST['mode'] ?? ''){
	case 'reply':
		if (isset($_POST['userid']) && ($_POST['userid'] == 'rgergerger' or preg_match("/^[a-z A-Z]+$/", $_POST['title'] ?? ''))){ // ?? '' 추가
	...
	case 'write':
		...
		if ( isset($dbinfo['db']) && $dbinfo['db'] == '2010kbintern' ){
			...
			if (isset($list_cateuid['uid'])){
				$sql_kbintern = "SELECT count(uid) as cnt FROM {$dbinfo['table']} WHERE cateuid = '".db_escape($list_cateuid['uid'])."' "; // db_escape 추가
				$list_kbintern = db_arrayone($sql_kbintern);
				if (isset($list_kbintern['cnt']) && $list_kbintern['cnt'] >= 200 ) back("선착순 200명 모집이 완료 되었습니다.성원에 감사드립니다.");
			}
		}
		//=====================================================================
	...
	case 'modify':
		...
		if(isset($_REQUEST['goto'])){
			...
			$goto = $thisUrl.'read.php?' . href_qs('uid='.($_REQUEST['uid'] ?? ''),$qs_basic); // ?? '' 추가
		}
...
function reply_ok(&$dbinfo, $qs){
	...
	// $qs 추가, 체크후 값 가져오기
	$qs['uid']	= 'post,trim,notnull=' . urlencode('답변할 게시물의 고유넘버가 넘어오지 않았습니다.'); // tirm -> trim 오타 수정
	$qs=check_value($qs);
	
	// 자동 글쓰기 방지 인증 ........ davej.......2014-10-25
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['captcha'])){
		if (empty($_SESSION['cap_code']) || $_POST['captcha'] != $_POST['captcha']){
		...
		foreach($fieldlist as $value){
			...
			switch($value){
				case 'gisu' :
					...
					break;
				case 'url' :
					$qs['url'] = check_url($_POST['url'] ?? ''); // isset(.. ? ..) 제거
					break;			
				// board2 reply
				case 're' : // re값 구하는 함수 호출
					$qs['re'] = userReplyRe($dbinfo['table'], $list['num'] ?? '', $list['re'] ?? ''); // ?? 추가
					break;
				case 'num' : // 부모 글과 동일
					$qs['num'] = $list['num'] ?? null; // ?? 추가
					break;
				case 'cateuid' : // 부모 글과 동일
					$qs['cateuid'] = $list['cateuid'] ?? null; // ?? 추가
					break;
				case 'catetitle' : // 부모 글과 동일
					$qs['catetitle'] = $list['catetitle'] ?? ''; // ?? 추가
					break;					
				case 'type' : // 부모 글과 동일
					$qs['type']	= $list['type'] ?? 'docu'; // ?? 추가
					break;
				
				// board2 write
				case 'priv_read' :
					if(($dbinfo['enable_privread'] ?? '') == 'Y'){ // ?? 추가
						$qs[$value] = str_replace(' ','',$_POST[$value] ?? ''); // ?? 추가
						$qs[$value] = str_replace(',,',',',$qs[$value]); // 콤머두개 콤머하나로
					}
					break;
				case 'priv_hidelevel' : // 정수값으로
					$qs['priv_hidelevel'] = (int)($_POST['priv_hidelevel'] ?? 0); // ?? 추가
					break;
				...
				case 'docu_type' : // html값이 아니면 text로
					if(!isset($_POST['docu_type']) || !$_POST['docu_type']){
						$_POST['docu_type'] = $dbinfo['default_docu_type'] ?? 'text'; // ?? 추가
					}
				...
				case 'userid' :
					if(isset($_SESSION['seUid'])){
						switch($dbinfo['enable_userid'] ?? ''){ // ?? 추가
					...
					} elseif ((($dbinfo['priv_reply'] ?? 0) > 0) && !isset($_SESSION['seUid'])){ // ?? 0 추가
			...
			if(isset($qs[$value])){
				if($value == 'passwd'){
					$sql_set .= ", passwd	=PASSWORD('".db_escape($qs['passwd'] ?? '')."') "; // ?? 추가
				} else {
					$sql_set .= ", {$value} = '" . db_escape($qs[$value]) . '\' ';
				}
			} elseif(isset($_POST[$value])){
				if($value == 'passwd'){
					$sql_set .= ", passwd	=PASSWORD('".db_escape($_POST['passwd'] ?? '')."') "; // ?? 추가
				} else {
	...
	if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] != 'N' and isset($_FILES)){
		...
		if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] == 'Y'){
			$POSTFILE['upfile'] = $_FILES['upfile']; // & 참조 제거
		} else {
			$POSTFILE = $_FILES; // & 참조 제거
		}
		...
		foreach($POSTFILE as $key => $value){
			if(isset($value['name']) && $value['name']) { // 파일이 업로드 되었다면
				...
				if( isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] == 'image'
					AND !is_array(@getimagesize($value['tmp_name'] ?? '')) ) // @, ?? 추가
					continue;
				$upfiles[$key]=file_upload($key,$updir);
				$upfiles_totalsize += $upfiles[$key]['size'] ?? 0; // ?? 추가
			}
		} // end foreach
	...
	if( (isset($dbinfo['enable_adm_mail']) && $dbinfo['enable_adm_mail'] == 'Y') or (isset($dbinfo['enable_rec_mail']) && $dbinfo['enable_rec_mail'] == 'Y') ){
		...
		$mail->subject	= '[게시판-답변] '. ($qs['title'] ?? ''); // ?? 추가
		if(isset($qs['docu_type']) && $qs['docu_type'] == 'html'){
			$mail->body	= "[". ($qs['userid'] ?? '') ."]님께서 다음과 같은 게시물을 남겼습니다.]<br><hr>제목:". ($qs['title'] ?? '') ."<hr>". ($qs['content'] ?? '') ."<hr>예상되는게시판주소:http://{$_SERVER['HTTP_HOST']}/sboard2/read.php?db=". ($_REQUEST['db'] ?? '') ."&uid={$uid}";
			$mail->html	= 1;
		} else {
			$mail->body	= "[". ($qs['userid'] ?? '') ."]님께서 다음과 같은 답변 게시물을 남겼습니다.]\n
제목:". ($qs['title'] ?? '') ."\n
--------------------------------------------\n
". ($qs['content'] ?? '') ."\n
--------------------------------------------
...
function write_ok(&$dbinfo, $qs){
	...
	// 같은글 등록 막기
	$user_id = $_POST['userid'] ?? ($_SESSION['seName'] ?? ''); // ?? 추가
	...
	// 자동 글쓰기 방지 인증 ........ davej.......2014-10-25
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['captcha'])){
		if (empty($_SESSION['cap_code']) || $_POST['captcha'] != $_POST['captcha']) back("자동 글쓰기 방지 인증 값이 맞지 않아 내용이 초기화 됩니다.");
	}
		...
		foreach($fieldlist as $value){
			...
			switch($value){
				...
				case 'ip' :
					...
					break;
				case 'num' :
					$sql = "SELECT max(num) FROM {$dbinfo['table']} where {$sql_where} "; // 공백 수정
				...
				case 'userid' :
					...
					// ✨ [오류 수정] : elseif ((... or ...)) && ...
					} elseif ( ( ($dbinfo['priv_write'] ?? '') == '회원' || ($dbinfo['priv_write'] ?? '') == '운영자' ) && !isset($_SESSION['seUid']) ){
							back("세션이 끊겼습니다. 로그인 후 사용이 가능합니다.");
					}
					break;
				case 'email' :
			...
			if(isset($qs[$value])){
				if($value == 'passwd'){
					...
					$sql_set .= ", {$value} = '" . db_escape($qs[$value] ?? '') . '\' '; // ?? 추가
				}
			} elseif(isset($_POST[$value])){
				if($value == 'passwd'){
					$sql_set .= ", passwd	=PASSWORD('".db_escape($_POST['passwd'] ?? '')."') "; // ?? 추가
				} else {
					$sql_set .= ", {$value} = '" . db_escape($_POST[$value] ?? '') . '\' '; // ?? 추가
				}
			}
		} // end foreach
	} // end if
	...
	if( isset($dbinfo['enable_adm_mail']) && $dbinfo['enable_adm_mail'] == 'Y' and isset($dbinfo['email']) && $dbinfo['email'] ){
		...
		$mail->name		= '게시판 자동메일';
		$mail->to		= $dbinfo['email'];
		$mail->subject	= '[게시판자동메일] '. ($qs['title'] ?? ''); // ?? 추가
		if(isset($qs['docu_type']) && $qs['docu_type'] == 'html'){
			$mail->body	= "[". ($qs['userid'] ?? '') ."]님께서 다음과 같은 게시물을 남겼습니다.]<br><hr>제목:". ($qs['title'] ?? '') ."<hr>". ($qs['content'] ?? '') ."<hr>예상되는게시판주소:http://{$_SERVER['HTTP_HOST']}/sboard2/read.php?db=". ($_REQUEST['db'] ?? '') ."&uid={$uid}";
			$mail->html	= 1;
		} else {
...
예상되는게시판URL: http://{$_SERVER['HTTP_HOST']}/sboard2/read.php?db=". ($_REQUEST['db'] ?? '') ."&uid={$uid}";
	...
	if( isset($_REQUEST['db']) && $_REQUEST['db'] == "cmmemo"){
		...
		// 이미 적립했는지
		$sql = "select uid from new21_account where bid='" . db_escape($_SESSION['seUid'] ?? '') . "' and rdate_date=curdate() and remark='{$remark}'"; // db_escape 추가
		if(!db_resultone($sql,0,'uid'))
			new21PointDeposit($_SESSION['seUid'] ?? 0 , 50, $remark ,'적립');
	}
	// 포인트 적립해줌
...
function modify_ok(&$dbinfo,$qs){
	...
	if(!privAuth($dbinfo,'priv_modify') ){
		if(isset($list['bid']) && $list['bid']>0){
			if( ($list['bid'] != ($_SESSION['seUid'] ?? 0)) or ('nobid' == substr($dbinfo['priv_modify'] ?? '',0,5)) ){ // ?? 추가
				back('수정하실 권한이 없습니다.');
			}
		} else {
...
//	$skip_fields = array( 'bid', 'num', 're', 'passwd', 'type', 'uid', 'upfiles', 'upfiles_totalsize', 'hit', 'hitip', 'hitdownload', 'vote', 'voteip', 'rdate');
		...
		foreach($fieldlist as $value){
			...
			switch($value){
				...
				case 'cateuid' : // catelist에서 선택한 값을
					...
					if(isset($_POST['catelist']) and strlen($list['re'] ?? '') == 0){ // ?? 추가
						$qs['cateuid'] = $_POST['catelist'];
					} elseif (isset($_POST['cateuid']) and strlen($list['re'] ?? '') == 0){ // ?? 추가
						$qs['cateuid'] = $_POST['cateuid'];
					} else {
						$qs['cateuid'] = $list['cateuid'] ?? null; // ?? 추가
					}
					if(isset($_POST['catelist']) && ($list['cateuid'] ?? null) != ($qs['cateuid'] ?? null)){ // ?? 추가
						$sw_change_cateuid = 1;
					}
					break;
				case 'catetitle' :
					...
					$sql="select title from {$dbinfo['table_cate']} where uid='".db_escape($qs['cateuid'] ?? '')."'"; // ?? 추가
					$qs['catetitle'] = db_resultone($sql,0,'title');
					break;					
				case 'priv_read' :
					if(isset($dbinfo['enable_privread']) && $dbinfo['enable_privread'] == 'Y'){
				...
				case 'title' : //davej.........모든 태그 삭제
					...
					break;				
				case 'docu_type' : // html값이 아니면 text로
					if(!isset($_POST['docu_type']) || !$_POST['docu_type']) $_POST['docu_type'] = $dbinfo['default_docu_type'] ?? 'text'; // ?? 추가
					$_POST['docu_type'] = strtolower($_POST['docu_type']);
					if($_POST['docu_type'] != 'html'){
						$_POST['docu_type']='text';
					}
	...
	if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] != 'N' and isset($_FILES) ){
		// 파일 업로드 드렉토리
		$updir = $dbinfo['upload_dir'] . '/' . (int)($list['bid'] ?? 0); // ?? 추가

		// 기존 업로드 파일 정보 읽어오기
		if(isset($list['upfiles']) && $list['upfiles']){
			$upfiles=@unserialize($list['upfiles']); // @ 추가
			if(!is_array($upfiles)) { // 시리얼화된 변수가 아닌 파일 명으로 되어 있을 경우
				$upfiles = []; // 배열로 초기화
				$upfiles['upfile']['name']=$list['upfiles'];
				$upfiles['upfile']['size']=(int)($list['upfiles_totalsize'] ?? 0); // ?? 추가
			}
		} else {
			$upfiles = [];
		}
		$upfiles_totalsize=(int)($list['upfiles_totalsize'] ?? 0); // ?? 추가
		...
		// 사용변수 초기화
		$POSTFILE	= array(); // 업로드 폼값 다시 저장
		if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] == 'Y') $POSTFILE['upfile'] = $_FILES['upfile']; // & 참조 제거
		else $POSTFILE = $_FILES; // & 참조 제거
		...
		foreach($POSTFILE as $key => $value){
			if(isset($value['name']) && $value['name']) { // 파일이 업로드 되었다면
				...
				if(isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] == 'image'
					AND !is_array(@getimagesize($_FILES[$key]['tmp_name'] ?? '')) ) // @, ?? 추가
					continue;
				...
				$upfiles_totalsize = $upfiles_totalsize - ($upfiles[$key]['size'] ?? 0) + ($upfiles_tmp['size'] ?? 0); // ?? 추가
	...
	$sql = "UPDATE {$dbinfo['table']} SET
				rdate	= " . ($list['rdate'] ?? 'UNIX_TIMESTAMP()') . "
				{$sql_set_file}
				{$sql_set}
			WHERE
				uid='{$uid_safe}'
	...
	if(isset($sw_change_cateuid) && $sw_change_cateuid ){
		db_query("update {$dbinfo['table']} set cateuid='".db_escape($qs['cateuid'] ?? '')."',catetitle='".db_escape($qs['catetitle'] ?? '')."' where num='".db_escape($list['num'] ?? '')."'"); // ??, db_escape 추가
	} // end if
...
function delete_ok(&$dbinfo){
	...
	if(!privAuth($dbinfo,'priv_delete')) {// 게시판 전체 삭제 권한을 가졌다면
		if( 'nobid' == substr($dbinfo['priv_delete'] ?? '',0,5) ) // ?? 추가
			back('삭제하실 수 없습니다.');
	...
	// 삭제
	db_query("DELETE FROM {$dbinfo['table']} WHERE uid='{$uid_safe}' and {$sql_where} "); // 공백 수정
	...
	if(isset($list['num']) && $list['num']){
		$num_safe = db_escape($list['num']);
		$re_safe = db_escape($list['re'] ?? '');
		$re_len_safe = strlen($list['re'] ?? '');
		
		$rs_subre = db_query("SELECT * FROM {$dbinfo['table']} WHERE num='{$num_safe}' AND length(re) > {$re_len_safe} AND locate('{$re_safe}',re) = 1");
		
		if ($rs_subre) { // 쿼리 성공 확인
			while($row=db_array($rs_subre)){
				if(isset($row['upfiles']) && $row['upfiles']){
					$updir = $dbinfo['upload_dir'] . '/' . (int)($row['bid'] ?? 0); // ?? 추가
					$upfiles=@unserialize($row['upfiles']); // @ 추가
					if(!is_array($upfiles)) { // 시리얼화된 변수가 아닌 파일 명으로 되어 있을 경우
						$upfiles = []; // 초기화
		...
		// 서브그룹도 삭제
		db_query("DELETE FROM {$dbinfo['table']} WHERE num='{$num_safe}' AND length(re) > {$re_len_safe} AND locate('{$re_safe}',re) = 1");
	} // end if	
...
function mode_vote(){
	...
	$vote_safe = (int)($qs['vote'] ?? 0);
	$uid_safe = db_escape($qs['uid'] ?? '');
	$remote_addr_safe = db_escape($_SERVER['REMOTE_ADDR'] ?? '');

	// 조회수 증가
	db_query("UPDATE {$dbinfo['table']} SET
					vote	=vote + {$vote_safe},
					voteip	='{$remote_addr_safe}'
				WHERE
					uid='{$uid_safe}'
				AND
					voteip<>'{$remote_addr_safe}'
	...
	if(db_count_affected_rows()) // 이 함수는 사용자 정의 함수 목록에 없었지만, 원본에 있으므로 유지합니다.
		back('성공적으로 참여하였습니다.');
	else
		back('이미 참여하셨습니다.');
...
function mode_memowrite(){
	...
	// 부모 게시물이 있는지 체크
	$pid_safe = db_escape($qs['pid'] ?? '');
	$sql = "select uid from {$dbinfo['table']} where uid='{$pid_safe}'";
...
function mode_memoModify(){
	...
	$pid_safe = db_escape($qs['pid'] ?? '');
	$passwd_safe = db_escape($_POST['passwd'] ?? '');
	$sql = "SELECT *,PASSWORD('{$passwd_safe}') as pass FROM {$dbinfo['table']}_memo WHERE uid='{$uid_safe}' and pid='{$pid_safe}' LIMIT 1";
	$list=db_arrayone($sql) or back('수정할 게시물이 없습니다. 확인 바랍니다.');
	...
	if($fieldlist = userGetAppendFields($dbinfo['table'].'_memo', $skip_fields)){
		foreach($fieldlist as $value){
			...
			// sql_set 만듦
			if(isset($qs[$value])) $sql_set .= ", {$value} = '" . db_escape($qs[$value] ?? '') . '\' '; // ?? 추가
			elseif(isset($_POST[$value])) $sql_set .= ", {$value} = '" . db_escape($_POST[$value] ?? '') . '\' '; // ?? 추가
		} // end foreach
	}
...
function mode_memodelete(){
	...
	// 삭제
	db_query("DELETE FROM {$dbinfo['table']}_memo WHERE uid='{$uid_safe}' ");
	...
	//elseif($dbinfo['goto_memowrite']) $goto = $dbinfo['goto_memowrite'];
		...
		$goto = $thisUrl.'read.php?' . href_qs('uid='.($_REQUEST['pid'] ?? ''),$qs_basic); // ?? 추가
	}
	back('',$goto);
...
function userReplyRe($table, $num, $re){
	...
	if (($dbinfo['table_name'] ?? '') != ($dbinfo['db'] ?? '')) {
		$sql_where = " db='".db_escape($dbinfo['db'] ?? '')."' "; // ??, db_escape 추가
	}
	...
	// ✨ [수정] : SQL 인젝션 방지를 위해 db_escape 적용
	$num_safe = db_escape($num);
	$re_safe = db_escape($re);
	$re_len_safe = strlen($re_safe);
	
	$sql = "SELECT re, right(re,1) FROM {$table} WHERE {$sql_where} and num='{$num_safe}' AND length(re)={$re_len_safe}+1 AND locate('{$re_safe}', re) = 1 ORDER BY re DESC LIMIT 1";
	$row = db_arrayone($sql);

...
if(!$total=db_count($rs_list)) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			if(isset($list['rdate']) && $list['rdate']>time()-3600*24) $list['enable_new']="Y";
			else $list['enable_new']="";

			$list['rdate_date']= isset($list['rdate']) && $list['rdate'] ? date("Y-m-d", $list['rdate']) : "";	//	날짜 변환
			
			$list['go_date']= isset($list['rdate']) && $list['rdate'] ? date("m-d", $list['rdate']) : "";	//	날짜 변환		

			$list['icrc_date']= isset($list['rdate']) && $list['rdate'] ? date("M	. d , Y", $list['rdate']) : "";	//	icrc 날짜 변환		
			
			if(!isset($list['title']) || !$list['title']) $list['title'] = "제목없음…";

			$list['content'] = replace_string($list['content'], $list['docu_type']);	// 문서 형식에 맞추어서 내용 변경
			$list['data1'] = replace_string(isset($list['data1']) ? $list['data1'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data2'] = replace_string(isset($list['data2']) ? $list['data2'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data3'] = replace_string(isset($list['data3']) ? $list['data3'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data4'] = replace_string(isset($list['data4']) ? $list['data4'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data5'] = replace_string(isset($list['data5']) ? $list['data5'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data6'] = replace_string(isset($list['data6']) ? $list['data6'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data7'] = replace_string(isset($list['data7']) ? $list['data7'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data8'] = replace_string(isset($list['data8']) ? $list['data8'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경
			$list['data9'] = replace_string(isset($list['data9']) ? $list['data9'] : '', isset($list['docu_type']) ? $list['docu_type'] : '');	// 문서 형식에 맞추어서 내용 변경

			//답변이 있을 경우 자리는 길이를 더 줄임
			$cut_length = isset($list['rede']) && $list['rede'] ? (isset($dbinfo['cut_length']) ? $dbinfo['cut_length'] : 0) - $list['rede'] -3 : (isset($dbinfo['cut_length']) ? $dbinfo['cut_length'] : 0);
			$list['cut_title'] = cut_string(isset($list['title']) ? $list['title'] : '', $cut_length);
...
$tpl->set_var('player'		,isset($player) ? $player : array());
// 템플릿 마무리 할당
...
$list['pid'] = isset($_GET['p_uid']) ? $_GET['p_uid'] : '';
$list['cateuid'] = isset($_REQUEST['cateuid']) ? $_REQUEST['cateuid'] : '';
$now = date("ymd", time());

$sql1 = "select rdate from new21_board2_monitoring where puid=". (isset($_GET['p_uid']) ? (int)$_GET['p_uid'] : 0) ." limit 0, 1";
$sql2 = "select rdate from new21_board2_medical where puid=". (isset($_GET['p_uid']) ? (int)$_GET['p_uid'] : 0) ." limit 0, 1";
$sql3 = "select rdate from new21_board2_education where puid=". (isset($_GET['p_uid']) ? (int)$_GET['p_uid'] : 0) ." limit 0, 1";
$sql4 = "select rdate from new21_board2_pain where puid=". (isset($_GET['p_uid']) ? (int)$_GET['p_uid'] : 0) ." limit 0, 1";
$sql5 = "select rdate from new21_board2_house where puid=". (isset($_GET['p_uid']) ? (int)$_GET['p_uid'] : 0) ." limit 0, 1";
...
$tpl->set_var('cateinfo'		,isset($cateinfo) ? $cateinfo : array());

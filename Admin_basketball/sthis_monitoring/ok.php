...
function write_ok(&$dbinfo, $qs){
	...
	//////////////////////////////////////
	// 카테고리 정보 가져와 dbinfo 재 설정
	if($dbinfo['enable_cate'] == 'Y' and $dbinfo['enable_cateinfo'] == 'Y' and $_REQUEST['catelist']){
		$dbinfo['table_cate']	= {$dbinfo['table']} . "_cate";
	...
	// E-Mail 전송
	if( $dbinfo['enable_adm_mail'] == 'Y' and $dbinfo['email'] ){
		$mail = new mime_mail;
...
function memoWrite_ok(&$dbinfo){
	...
	// 권한 검사
	if($dbinfo['enable_memo'] != 'Y' || !privAuth($dbinfo, 'priv_memo'))
		back('메모를 쓸 수 없습니다.');

...
//	DATE		수정인			수정 내용
// --------	----------	--------------------------------------
// 25/08/11	Gemini AI	PHP 7.4+ 호환성 업데이트, MySQLi 적용, 보안 강화 (Prepared Statements)
...
if(($mode === "modify" || $mode === "reply") && $uid && $num){
	// SECURITY: SQL 인젝션 방지를 위해 Prepared Statement 사용
	// WARNING: MySQL의 password() 함수는 오래되어 보안에 취약합니다. password_hash() 사용을 권장합니다.
	$stmt = $mysqli->prepare("SELECT *, password(rdate) as private_key FROM {$dbinfo['table']} WHERE uid=? AND num=?");
...
$re_readlist	= $mysqli->query($sql);

$dbinfo['row_pern']		= $dbinfo['row_pern'] ?? 8;

if(!$re_readlist || $re_readlist->num_rows === 0) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			$readlist		= $re_readlist->fetch_assoc();
...
if (isset($list['p_num']))	$list['numimages'] = "<img src='images/savers_team_num".$list['p_num'].".gif'>";
	...
	if ($puid > 0) {
		...
		foreach($tables as $key => $table) {
			$stmt = $mysqli->prepare("SELECT rdate FROM {$table} WHERE puid=? ORDER BY rdate DESC LIMIT 1");
			$stmt->bind_param("i", $puid);
			$stmt->execute();
			$result = $stmt->get_result();
			if($db_date = $result->fetch_assoc()){
...
if(preg_match("/^(ht|h|t)$/",$dbinfo['html_headpattern'] ?? '')){
	$HEADER['header'] = 2; // == 는 오타일 가능성이 높아 = 로 수정

...
if(($_REQUEST['mode'] ?? '') == "delete") $_REQUEST['puid'] = $_GET['p_uid'] ?? null;
if(empty($_REQUEST['puid'])) back("고유번호가 없습니다.");
...
function reply_ok(&$dbinfo, $qs){
	...
	// 부모글 가져오기
	$sql = "SELECT * FROM {$dbinfo['table']} WHERE uid='".db_escape($qs['uid']) . "'";
...
function modify_ok(&$dbinfo,$qs,$field){
	...
	if( (int)($qs['cateuid'] ?? 0) <> (int)($list['cateuid'] ?? 0) ){
...
function delete_ok(&$dbinfo,$field,$goto){
	...
	if(!privAuth($dbinfo,"priv_delete")) {// 게시판 전체 삭제 권한을 가졌다면
		if(privAuth($dbinfo,"priv_writer")) { // 작성자에게 권한이 있으면
			if(($list['bid'] ?? 0) == 0 and $list['passwd'] != $list['pass']){
				if(isset($_SERVER['QUERY_STRING']))
					back("비밀번호를 입력하여 주십시오","{$thisUrl}/delete.php?{$_SERVER['QUERY_STRING']}");
				else back("비밀번호를 정확히 입력하십시오");
			} elseif (($list['bid'] ?? 0) > 0 and $list['bid'] != ($_SESSION['seUid'] ?? '')) back("삭제할 권한이 없습니다.");
		}
	...
	// 답변글과 파일도 함께 삭제 준비
	if(isset($list['num'])){
		$sql = "SELECT * FROM {$dbinfo['table']} WHERE num='".db_escape($list['num']) . "' AND length(re) > length('".db_escape($list['re']) . "') AND locate('".db_escape($list['re']) . "',re) = 1";
		$rs_subre = db_query($sql);

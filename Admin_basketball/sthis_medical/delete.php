...
if(isset($_GET['db'])){
	// 중요: $_GET['db'] 값을 직접 쿼리에 사용하는 것은 SQL 인젝션에 취약할 수 있습니다.
	// 사용자 정의 DB 함수 내에서 반드시 이스케이프 처리가 필요합니다.
...
if(isset($_GET['mode']) && $_GET['mode'] == "memo") // memo 삭제 요청의 경우(하지만 메모는 로그인한사람만 쓸 수 있음)
...
// 마무리
// PHP 7+ 호환성을 위해 preg_replace의 백 레퍼런스를 $1로 변경
$val="\$1{$thisUrl}/stpl/{$dbinfo['skin']}/images/";
// - 사이트 템플릿 읽어오기
// PHP 7+ 호환성을 위해 ereg()를 preg_match()로 변경 및 버그 수정 ($HEADER['header'] == 2 -> = 2)

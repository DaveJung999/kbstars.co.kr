...
$cateuid = isset($cateuid) ? $cateuid : (isset($_REQUEST['cateuid']) ? $_REQUEST['cateuid'] : '');
$db = isset($db) ? $db : '';
$pern = isset($pern) ? $pern : '';
$sc_column = isset($sc_column) ? $sc_column : '';
$sc_string = isset($sc_string) ? $sc_string : '';
...
if(!$total=db_count($re_readlist)) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			// new image넣을 수 있게 <opt name="enable_new">..
			if(isset($readlist['rdate']) && $readlist['rdate']>time()-3600*24) $readlist['enable_new']="<img src='/images/icon_new.gif' width='30' height='15' border='0'>";

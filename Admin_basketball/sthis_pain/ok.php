...
function reply_ok(&$dbinfo, $qs){
	...
	// E-Mail 전송
	if( ($dbinfo['enable_adm_mail'] == 'Y' or $dbinfo['enable_rec_mail'] == 'Y') and isset($dbinfo['email']) ){
		if($dbinfo['enable_rec_mail'] == 'Y'){
			// dbinfo['email']에 부모글 메일 주소 추가
			if($list['email'] = check_email($list['email'])){
				if($dbinfo['email']) $dbinfo['email'] .= ",{$list['email']}";
...
function write_ok(&$dbinfo, $qs){
	...
	//////////////////////////////////////
	// 카테고리 정보 가져와 dbinfo 재 설정
	if($dbinfo['enable_cate'] == 'Y' and $dbinfo['enable_cateinfo'] == 'Y' and isset($_REQUEST['catelist'])){
		$dbinfo['table_cate']	= {$dbinfo['table']} . "_cate";

		$sql = "select * from {$dbinfo['table_cate']} where uid='{$_REQUEST['catelist']}' LIMIT 1";
		$cateinfo = db_arrayone($sql);
			...
			switch($value){
				...
				case 'catetitle' :
					if(isset($_POST['catelist'])){
						$sql="select title from {$dbinfo['table_cate']} where uid='{$_POST['catelist']}'";
...
function modify_ok(&$dbinfo,$qs,$field){
	...
	// 수정할 글 읽기
	$sql = "SELECT *,password('{$_POST['passwd']}') as postpasswd FROM {$dbinfo['table']} WHERE {$field}='{$qs[$field]}'";
	...
	if( (int)$qs['cateuid'] <> $list['cateuid'] ){
		db_query("update {$dbinfo['table']} set cateuid='{$qs['cateuid']}',catetitle='{$qs['catetitle']}' where num='{$list['num']}'");
	} // end if
...
function memoWrite_ok(&$dbinfo){
	global $db_conn, $_SESSION;
	// 권한 검사
	if( $dbinfo['enable_memo'] != 'Y' or !privAuth($dbinfo, 'priv_memo') )
		back('메모를 쓸 수 없습니다.');
...
function memodelete_ok(&$dbinfo,$field){
	...
	// 삭제 권한 체크와 해당 게시물 읽어오기
	$sql = "SELECT *,password('{$qs['passwd']}') as pass FROM {$dbinfo['table']} WHERE {$field}='{$qs[$field]}' and pid='{$qs['pid']}'";
	$list = db_arrayone($sql) or back("이미 삭제되었거나 잘못된 요청입니다");
	if(!privAuth($dbinfo,"priv_delete")) {// 게시판 전체 삭제 권한을 가졌다면
		if($list['bid'] == 0 and $list['passwd'] != $list['pass']){
			if(isset($_SERVER['QUERY_STRING']))

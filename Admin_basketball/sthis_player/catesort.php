	...
	if($_POST['mode'] == "change"){
		...
		if(strlen($src['re'])) {
			if($_POST['dstuid']=="first") { // 처음으로 이동한 경우 (re=ab라면 a보다 크고 ab보다 작은 범위를 1씩 증가후 re=a1으로 변경)
				...
				if($dst['re'] == substr($src['re'],0,-1)."1" ) 
					db_query("update {$table_cate} 
								SET re=concat( substring(re,1,{$src['length']}-1), char(ord(substring(re,{$src['length']},1))+1 ), substring(re,{$src['length']}+1) ) 
								where {$sql_where_cate} and num='{$src['num']}' 
								and strcmp(re,substring(re,1,{$src['length']}-1))>0 
								and strcmp(re,'{$src['re']}')< 0"); // dst[re]의 맨뒤가 '1'이 아니면 구지 1씩 증가시킬 필요 없겠지^^;;
				db_query("update {$table_cate} 
								SET re=concat( substring(re,1,{$src['length']}-1), '1', substring(re,{$src['length']}+1)) 
								where {$sql_where_cate} and {$sql_where_srcuid_in}"); // src[re]의 마지막은 '1'부터 시작하니 강제로 '1'로 해야겠지^^
			}
			else {
				...
				if( strcmp($src['re'],$dst['re']) > 0 ){ // 상위로 이동할 경우 ( 목적위치+1이상에서 본래위치 미만 범위를 1씩 증가후 본래위치는 목적위치+1
					...
					db_query("update {$table_cate} 
								SET re=concat( substring(re,1,{$src['length']}-1), char(ord(substring(re,{$src['length']},1))+1 ), substring(re,{$src['length']}+1) ) 
								where {$sql_where_cate} and num='{$src['num']}' 
								and strcmp(re,'{$dst['re_next']}')>=0 
								and strcmp(re,'{$src['re']}')< 0 ");
					db_query("update {$table_cate} 
								SET re=concat( substring(re,1,{$src['length']}-1), right('{$dst['re_next']}',1), substring(re,{$src['length']}+1)) 
								where {$sql_where_cate} and {$sql_where_srcuid_in}");
				}
				elseif(strcmp($src['re'],$dst['re']) < 0) { // 하위로 이동할 경우 ( 본래위치+1이상에서 목적위치+1 미만 범위를 1씩 감소후 본래위치는 목적위치 
					...
					db_query("update {$table_cate} 
								SET re=concat( substring(re,1,{$src['length']}-1), char(ord(substring(re,{$src['length']},1))-1 ), substring(re,{$src['length']}+1) ) 
								where {$sql_where_cate} and num='{$src['num']}' 
								and strcmp(re,'{$src['re_next']}')>= 0 
								and strcmp(re,'{$dst['re_next']}')<0 ");
					db_query("update {$table_cate} 
								SET re=concat( substring(re,1,{$src['length']}-1), right('{$dst['re']}',1) , substring(re,{$src['length']}+1) ) 
								where {$sql_where_cate} 
								and {$sql_where_srcuid_in}");
				}
			}
		}
			...
			if($_POST['dstuid']=="first") { // 최상위로 이동할 경우
				...
				if($dst['num']==1) 
					db_query("update {$table_cate} 
								SET num=num+1 
								WHERE {$sql_where_cate} and num < {$src['num']}"); // dst[num]이 1일 아니라면 키울필요 없겠지^^
				db_query("update {$table_cate} 
							SET num=1 
							WHERE {$sql_where_cate} and {$sql_where_srcuid_in}"); // 처음값이기때문에 dst[num]보다 1로 변경함..
			}
			else {
...
?>
	...
	<table border=0 cellspacing='1' cellpadding='3' bgcolor='black' width=350 height=100>
		...
		<tr>
			<td bgcolor='#CCFFCC'>현재 <b><?php echo $cateinfo['title'] ?></b> 메뉴입니다.</td>
		</tr>
		<tr>
			<td bgcolor='#EDFEE'><?php echo $list['menu'] ?> 메뉴를 
			<select name="dstuid">
<?php

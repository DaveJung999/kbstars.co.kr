...
$table	= "{$SITE['th']}slist_player_league"; // new21_slist_event
...
switch($_REQUEST['mode']){
	case 'write':
		$puid = write_ok($table, $qs);
		go_url(isset($_REQUEST['goto']) ? $_REQUEST['goto'] : "{$thisUrl}/read.php?uid={$puid}");
		break;
	case 'reply':
		...
		break;
	case 'modify':
		$puid = modify_ok($table,$qs,"puid");
		go_url(isset($_REQUEST['goto']) ? $_REQUEST['goto'] : "{$thisUrl}/read.php?uid={$puid}");
		break;
	case 'delete':
		$goto = isset($_REQUEST['goto']) ? $_REQUEST['goto'] : "{$thisUrl}/read.php?uid={$uid}";
		delete_ok($table,"puid",$goto);
		go_url($goto);
		break;
	// VOTE
	case 'vote' :
...
function reply_ok($table,$qs){
	...
	/////////////////////////////////
	$sql_set_file = '';
	if(isset($dbinfo['enable_upload']) and $dbinfo['enable_upload'] != 'N' and isset($_FILES)){
...
function delete_ok($table,$field,$goto){
	...
	db_query("DELETE FROM {$table} where p_uid='{$qs['puid']}'");

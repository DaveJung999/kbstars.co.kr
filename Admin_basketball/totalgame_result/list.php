...
if(!(isset($_GET['skin']) && $_GET['skin'] == "")){
	$table = isset($_REQUEST['db']) ? $_REQUEST['db'] : (isset($table) ? $table : '');
...
// misc
//=====
$sql_cnt = "SELECT count(*) AS CNT FROM {$table} WHERE  $sql_where ";

// 페이지 나눔등 각종 카운트 구하기
$count['total']=db_resultone($sql_cnt, 0, "CNT"); // 전체 게시물 수
// 게시물 일부만 본다면
...
$sql_today = "SELECT count(*) AS CNT FROM {$table} WHERE (rdate > unix_timestamp(curdate())) and $sql_where ";
$count['today']=db_resultone($sql_today , 0, "CNT");
...
if(!$total=db_count($rs_list)) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		if(isset($dbinfo['row_pern'])) $tpl->set_var('CELL',"");
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴			
			...
			$list['rdate']= $list['rdate'] ? date("Y.m.d", (int)$list['rdate']) : "";	//	날짜 변환

...
//=======================================================
	...
	while( $list=db_array($result) ) {
		...
		// XSS 방지를 위해 htmlspecialchars 처리
		$js_title = htmlspecialchars($list['title'], ENT_QUOTES, 'UTF-8');
		$js_lhour = htmlspecialchars($lhour, ENT_QUOTES, 'UTF-8');
		$js_content = htmlspecialchars($list['content'], ENT_QUOTES, 'UTF-8');

		$outCal[$list['startdate']] .= "<img src={$thisUrl}/images/micon.gif border=0><font face=굴림><span style='font-size:9pt'><a href='{$href['view']}' onMouseOver=\"view('{$js_title}', '{$js_lhour}','{$js_content}');\" onMouseOut=\"noview();\">" . htmlspecialchars($list['title'], ENT_QUOTES) . "</a></span></font><br> \n"	;
	} // end while
	////////////////////////////
	...
	while( $list=db_array($result) ) {
		...
		while($tmp_time<=$tmp_time_enddate) {// 말일을 지나기 전까지
			...
			$outCal[$tmp] .= "<img src={$thisUrl}/images/micon.gif border=0><font face=굴림><span style='font-size:9pt'><a href='{$href['view']}' onMouseOver=\"view('{$js_title}', '{$js_lhour}','{$js_content}');\" onMouseOut=\"noview();\">" . htmlspecialchars($list['title'], ENT_QUOTES) . "</a></span></font><br> \n"	;
...
for($intNextWeek=1; $intNextWeek < 7 ; $intNextWeek++) { //주단위 루프 시작, 최대 6주
	...
	for($intNextDay=1; $intNextDay < 8 ; $intNextDay++) { //요일단위 루프 시작, 일요일부터
		...
		if ($intPrintDay==1 && $intNextDay<$intFirstWeekday+1) { //첫주시작일이 1보다 크면p?
			...
			if ($intPrintDay > $intLastDay ) { //입력날짜가 월말보다 크다면
				...
				if( $intThisYear-$NowThisYear==0 && $intThisMonth-$NowThisMonth==0 && $intPrintDay-$NowThisDay==0 ) {
					//오늘 날짜이면은 글씨폰트를 다르게
					//if($enable_write or isset($outCal[$intcday]))
				...
				elseif( $intNextDay==1 ) {
					//일요일이면 빨간 색으로
					if(isset($outCal[$intcday]))
						echo "<b><a href='{$href['goday']}'><font size=2 color='#5167DF'>{$intPrintDay}</font></a></b>\n";
					else
						echo "<font size=2 color='#5167DF'>{$intPrintDay}</font>\n";
				}
				else{
					// 그외의 경우
					if(isset($outCal[$intcday]))
						echo "<b><font size=2 color=black><a href='{$href['goday']}'>{$intPrintDay}</a></font></b>\n";
					else
						echo "<font size=2 color=black>{$intPrintDay}</font>\n";
				}

				// 일정 내용 출력
				//if(isset($outCal[$intcday])) echo $outCal[$intcday];

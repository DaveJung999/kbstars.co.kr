...
// page_security("", $_SERVER['HTTP_HOST']); // 이 함수가 정의되지 않아 주석 처리함.
...
// 기본 URL QueryString
$qs_basic = "db=" . db_escape($_REQUEST['db'] ?? $table) .			//table 이름
			"&mode=" . db_escape($_REQUEST['mode'] ?? '') .		// mode값은 list.php에서는 당연히 빈값
			"&cateuid=" . db_escape($_REQUEST['cateuid'] ?? '') .		//cateuid
			"&team=" . db_escape($_REQUEST['team'] ?? '') .				// 페이지당 표시될 게시물 수
			"&pern=" . db_escape($_REQUEST['pern'] ?? '') .				// 페이지당 표시될 게시물 수
			"&sc_column=" . db_escape($_REQUEST['sc_column'] ?? '') .	//search column
			"&sc_string=" . urlencode(stripslashes($sc_string ?? '')) . //search string
			"&page=" . db_escape($_REQUEST['page'] ?? '');

if( isset($_POST['db']) ) $dbinfo['db'] = db_escape($_POST['db']);

$dbinfo['upload_dir'] = trim($dbinfo['upload_dir'] ?? '') ? trim($dbinfo['upload_dir']) . "/" . db_escape($table) : dirname(__FILE__) . "/upload/" . db_escape($table) ;
...
function popWrite_ok(){
	...
	$sql_set=", `skin`		='" . db_escape($qs['skin'] ?? '') . "',
				`data0`		='400',
				`data1`		='400',
				`data2`		='no',
				`data3`		='yes',
...
function popModify_ok(){
	...
	// 수정할 글 읽기
	$sql = "SELECT * FROM {$table_popup} WHERE uid='" . db_escape($uid) . "'";
	...
	if( ($dbinfo['enable_upload'] ?? 'N') != 'N' && isset($_FILES) ){
		...
		if(is_array($upfiles) and count($upfiles)>0){
			foreach($upfiles as $key => $value){
				if(isset($_REQUEST["del_{$key}"])) {
						// 해당 파일 삭제
						if( isset($dbinfo['upload_dir'], $list['bid'], $upfiles[$key]['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($upfiles[$key]['name'])) ){
							@unlink($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($upfiles[$key]['name']));
							@unlink($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($upfiles[$key]['name']).".thumb.jpg"); // thumbnail 삭제
						}
						elseif( isset($dbinfo['upload_dir'], $upfiles[$key]['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name'])) ){
							@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name']));
							@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name']).".thumb.jpg"); // thumbnail 삭제
						}
		...
		if($dbinfo['enable_upload'] == 'Y') { // 파일 하나 업로드라면
			if(isset($_FILES['upfile']['name']) && $_FILES['upfile']['name']) {	// 파일이 업로드 되었다면
				...
				if($ok_upload){
					// 기존 업로드 파일이 있다면 삭제
					if( isset($dbinfo['upload_dir'], $list['bid'], $upfiles['upfile']['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($upfiles['upfile']['name'])) ){
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($upfiles['upfile']['name']));
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($upfiles['upfile']['name']).".thumb.jpg"); // thumbnail 삭제
					}
					elseif( isset($dbinfo['upload_dir'], $upfiles['upfile']['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name'])) ){
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name']));
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name']).".thumb.jpg"); // thumbnail 삭제
					}
					// 업로드
		...
		if(isset($dbinfo['enable_uploadmust']) && $dbinfo['enable_uploadmust'] == 'Y' && sizeof($upfiles) == 0){
			if(isset($dbinfo['enable_uploadextension']))
				back("다음의 파일 확장자만 업로드 가능합니다.\\n" . db_escape($dbinfo['enable_uploadextension']));
			if( isset($dbinfo['enable_upload']) && $dbinfo['enable_upload'] == 'image')
				back("이미지파일을 선택하여 업로드하여 주시기 바랍니다");
			else back("파일이 업로드 되지 않았습니다");
		}
		$sql_set_file = ", upfiles='".db_escape(serialize($upfiles)) . "', upfiles_totalsize='" . db_escape($upfiles_totalsize) . "' ";
	} // end if
	///////////////////////////////
	$title = $qs['title'] ?? '';
	$content = $qs['content'] ?? '';
	...
	$sql="UPDATE
			...
			SET
				...
				passwd	=PASSWORD('" . db_escape($passwd) . "'),
...
function popDelete_ok(){
	...
	if(privAuth($dbinfo,"priv_delete")) // 게시판 전체 삭제 권한을 가졌다면
		$rs_delete=db_query("SELECT * FROM {$table_popup} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "'");
	elseif(isset($_SESSION['seUid'])) // 회원의 글이라면,
		$rs_delete=db_query("SELECT * FROM {$table_popup} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "' and bid='" . db_escape($_SESSION['seUid']) . "'");
	else { // 비회원의 글이라면
		if(isset($qs['passwd']) && $qs['passwd'])
			$rs_delete=db_query("SELECT * FROM {$table_popup} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "' and passwd=PASSWORD('" . db_escape($qs['passwd']) . "')");
		else
			go_url("./delete.php?" . href_qs("uid=" . db_escape($qs['uid']),$qs_basic),0,"비밀번호를 입력하십시오"); // 비밀번호 입력 페이지로 이동
	} // end if
	...
	if(isset($list['upfiles'])){
		...
		foreach($upfiles as $key => $value){
			if(isset($value['name'])){
				if( isset($dbinfo['upload_dir']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($value['name'])) )
					$del_uploadfile[] = $dbinfo['upload_dir'] . "/" . db_escape($value['name']);
				elseif( isset($dbinfo['upload_dir']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($value['name'])) )
					$del_uploadfile[] = $dbinfo['upload_dir'] . "/" . db_escape($value['name']);
			} // end if
		} // end foreach
	} // end if

	$rs_subre = db_query("SELECT * FROM {$table_popup} WHERE $sql_where and num='" . db_escape($list['num']) . "' AND LENGTH(re) > LENGTH('" . db_escape($list['re']) . "') AND LOCATE('" . db_escape($list['re']) . "',re) = 1");
	...
	db_query("DELETE FROM {$table_popup} WHERE $sql_where and num='" . db_escape($list['num']) . "' AND LENGTH(re) > LENGTH('" . db_escape($list['re']) . "') AND LOCATE('" . db_escape($list['re']) . "',re) = 1");
	db_query("DELETE FROM {$table_popup} where $sql_where and uid='" . db_escape($list['uid']) . "'");
...
function modify_ok(){
	...
	if(privAuth($dbinfo,"priv_delete")) // 게시판 전체 삭제 권한을 가졌다면 수정 권한 무조건 부여
		$rs_modify=db_query("SELECT * FROM {$table} WHERE uid='" . db_escape($qs['uid']) . "'");
	elseif(isset($_SESSION['seUid'])) // 회원의 글이라면,
		$rs_modify=db_query("SELECT * FROM {$table} WHERE uid='" . db_escape($qs['uid']) . "' and bid='" . db_escape($_SESSION['seUid']) . "'");
	else { // 비회원의 글이라면 (비회원의 글에 패스워드가 없을 경우 누구든지 수정 가능, 실수로 안 입력했을 경우 수정가능하게)
		$rs_modify=db_query("SELECT * FROM {$table} WHERE uid='" . db_escape($qs['uid']) . "' and passwd=PASSWORD('" . db_escape($qs['passwd'] ?? '') . "')");
	} // end if
	...
	if(($dbinfo['enable_upload'] ?? 'N') != 'N' && isset($_FILES)){
		...
		if(($dbinfo['enable_upload'] ?? 'N') == 'Y') { // 파일 하나 업로드라면
			if(isset($_FILES['upfile']['name']) && $_FILES['upfile']['name']) { // 파일이 업로드 되었다면
				...
				// 기존 업로드 파일이 있다면 삭제
				if( isset($dbinfo['upload_dir'], $upfiles['upfile']['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name'])) )
					@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name']));
				elseif( isset($dbinfo['upload_dir'], $upfiles['upfile']['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name'])) )
					@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name']));
			...
			foreach($_FILES as $key => $value){
				if(isset($_FILES[$key]['name']) && $_FILES[$key]['name']) { // 파일이 업로드 되었다면
					...
					// 기존 업로드 파일이 있다면 삭제
					if( isset($dbinfo['upload_dir'], $upfiles[$key]['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name'])) )
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name']));
					elseif( isset($dbinfo['upload_dir'], $upfiles[$key]['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name'])) )
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name']));
	...
	if ( $content != 'banner'){
		//사용함 값 초기화
		$sql_data = "UPDATE {$table} SET data1 = '' where content = '" . db_escape($content) . "' ";
		db_query($sql_data);
	}
	$sql = "UPDATE
				{$table}
			SET
				...
				rdate	=UNIX_TIMESTAMP(),
				ip		='" . db_escape($qs['ip']) . "',
				priv_level	='" . db_escape($qs['priv_level'] ?? 0) . "'
				{$sql_set_file}
				{$sql_set}
			WHERE
...
function delete_ok(){
	...
	// 삭제 권한 체크와 해당 게시물 읽어오기
	if(privAuth($dbinfo,"priv_delete")) // 게시판 전체 삭제 권한을 가졌다면
		$rs_delete=db_query("SELECT * FROM {$table} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "'");
	elseif(isset($_SESSION['seUid'])) // 회원의 글이라면,
		$rs_delete=db_query("SELECT * FROM {$table} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "' and bid='" . db_escape($_SESSION['seUid']) . "'");
	else { // 비회원의 글이라면
		if(isset($qs['passwd']) && $qs['passwd'])
			$rs_delete=db_query("SELECT * FROM {$table} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "' and passwd=PASSWORD('" . db_escape($qs['passwd']) . "')");
		else
			go_url("./delete.php?" . href_qs("uid=" . db_escape($qs['uid']),$qs_basic),0,"비밀번호를 입력하십시오"); // 비밀번호 입력 페이지로 이동
	} // end if
	...
	$rs_subre = db_query("SELECT * FROM {$table} WHERE $sql_where and num='" . db_escape($list['num']) . "' AND LENGTH(re) > LENGTH('" . db_escape($list['re']) . "') AND LOCATE('" . db_escape($list['re']) . "',re) = 1");
	while($row=db_array($rs_subre)){
		if(isset($row['upfiles'])){
			$upfiles=unserialize($row['upfiles']);
			if(!is_array($upfiles)) { // 시리얼화된 변수가 아닌 파일 명으로 되어 있을 경우
	...
	db_query("DELETE FROM {$table} WHERE $sql_where and num='" . db_escape($list['num']) . "' AND LENGTH(re) > LENGTH('" . db_escape($list['re']) . "') AND LOCATE('" . db_escape($list['re']) . "',re) = 1");
	db_query("DELETE FROM {$table} where $sql_where and uid='" . db_escape($list['uid']) . "'");
...
function mailmessage_ok(){
	...
	if($fieldlist = userGetAppendFields($table_mail_message, $skip_fields)){
		foreach($fieldlist as $value){
			if(isset($_POST[$value])) $sql_set .= ", {$value} = '" . db_escape($_POST[$value]) . "' ";
		}
	}
	////////////////////////////////
	$sql = "UPDATE
...
function modify_sitedesign_ok(){
	...
	// 수정 권한 체크와 해당 게시물 읽어오기
	if(privAuth($dbinfo,"priv_delete")) // 게시판 전체 삭제 권한을 가졌다면 수정 권한 무조건 부여
		$rs_modify=db_query("SELECT * FROM {$table_sitedesign} WHERE uid='" . db_escape($qs['uid']) . "'");
	elseif(isset($_SESSION['seUid'])) // 회원의 글이라면,
		$rs_modify=db_query("SELECT * FROM {$table_sitedesign} WHERE uid='" . db_escape($qs['uid']) . "' and bid='" . db_escape($_SESSION['seUid']) . "'");
	else { // 비회원의 글이라면 (비회원의 글에 패스워드가 없을 경우 누구든지 수정 가능, 실수로 안 입력했을 경우 수정가능하게)
		$rs_modify=db_query("SELECT * FROM {$table_sitedesign} WHERE uid='" . db_escape($qs['uid']) . "' and passwd=PASSWORD('" . db_escape($qs['passwd'] ?? '') . "')");
	} // end if
	$list = db_count() ? db_array($rs_modify) : back("게시물이 없거나 수정할 권한이 없습니다");
	...
	if(($dbinfo['enable_upload'] ?? 'N') != 'N' && isset($_FILES)){
		...
		// 파일 업로드
		$updir = ($dbinfo['upload_dir'] ?? '') . "/" . db_escape($list['bid'] ?? 0) . "/";
		if(($dbinfo['enable_upload'] ?? 'N') == 'Y') { // 파일 하나 업로드라면
			if(isset($_FILES['upfile']['name']) && $_FILES['upfile']['name']) { // 파일이 업로드 되었다면
				$upfiles_tmp=file_upload("upfile",$updir);

				// 기존 업로드 파일이 있다면 삭제
				if( isset($dbinfo['upload_dir'], $upfiles['upfile']['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($list['bid'] ?? 0) . "/" . db_escape($upfiles['upfile']['name'])) )
					@unlink($dbinfo['upload_dir'] . "/" . db_escape($list['bid'] ?? 0) . "/" . db_escape($upfiles['upfile']['name']));
				elseif( isset($dbinfo['upload_dir'], $upfiles['upfile']['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name'])) )
					@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles['upfile']['name']));
			...
			foreach($_FILES as $key => $value){
				if(isset($_FILES[$key]['name']) && $_FILES[$key]['name']) { // 파일이 업로드 되었다면
					...
					// 기존 업로드 파일이 있다면 삭제
					if( isset($dbinfo['upload_dir'], $upfiles[$key]['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($list['bid'] ?? 0) . "/" . db_escape($upfiles[$key]['name'])) )
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($list['bid'] ?? 0) . "/" . db_escape($upfiles[$key]['name']));
					elseif( isset($dbinfo['upload_dir'], $upfiles[$key]['name']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name'])) )
						@unlink($dbinfo['upload_dir'] . "/" . db_escape($upfiles[$key]['name']));
	...
	$sql = "UPDATE
			...
			SET
				...
				rdate	=UNIX_TIMESTAMP(),
				ip		='" . db_escape($ip) . "',
				priv_level	='" . db_escape($priv_level) . "'
				{$sql_set_file}
				{$sql_set}
			WHERE
				uid='" . db_escape($uid_sitedesign) . "'
		";
	db_query($sql);
	...
	if( $cateuid <> ($list['cateuid'] ?? '') ){
		$type = $list['type'] ?? '';
		$num = $list['num'] ?? 0;
		db_query("update {$table_sitedesign} set cateuid='" . db_escape($cateuid) . "' where type='" . db_escape($type) . "' and num='" . db_escape($num) . "'");
	} // end if
...
function modify_logo_ok(){
	...
	if(($dbinfo['enable_upload'] ?? 'N') != 'N' && isset($_FILES)){
		...
		// 파일 업로드
		$updir = ($dbinfo['upload_dir'] ?? '') . "/" . db_escape($list['bid'] ?? '') . "/";
		if(($dbinfo['enable_upload'] ?? 'N') == 'Y') { // 파일 하나 업로드라면
			if(isset($_FILES['upfile']['name']) && $_FILES['upfile']['name']) { // 파일이 업로드 되었다면
				$upfiles_tmp=file_upload("upfile",$updir);
	...
	if( $cateuid <> ($list['cateuid'] ?? '') ){
		$type = $list['type'] ?? '';
		$num = $list['num'] ?? 0;
		db_query("update {$table} set cateuid='" . db_escape($cateuid) . "' where	type='" . db_escape($type) . "' and num='" . db_escape($num) . "'");
	} // end if
...
function write_banner_ok(){
	...
	//===========
	if(($dbinfo['enable_upload'] ?? 'N') != 'N' && isset($_FILES)){
		$updir = ($dbinfo['upload_dir'] ?? '') . "/" . db_escape($_SESSION['seUid'] ?? '') . "/";
...
function delete_banner_ok(){
	...
	if(isset($list['upfiles'])){
		...
		foreach($upfiles as $key => $value){
			if(isset($value['name'])){
				if( isset($dbinfo['upload_dir'], $list['bid']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($value['name'])) )
					$del_uploadfile[] = $dbinfo['upload_dir'] . "/" . db_escape($list['bid']) . "/" . db_escape($value['name']);
				elseif( isset($dbinfo['upload_dir']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($value['name'])) )
					$del_uploadfile[] = $dbinfo['upload_dir'] . "/" . db_escape($value['name']);
			} // end if
		} // end foreach
	...
	while($row=db_array($rs_subre)){
		if(isset($row['upfiles'])){
			...
			foreach($upfiles as $key => $value){
				if(isset($value['name'])){
					if( isset($dbinfo['upload_dir'], $row['bid']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($row['bid']) . "/" . db_escape($value['name'])) )
						$del_uploadfile[] = $dbinfo['upload_dir'] . "/" . db_escape($row['bid']) . "/" . db_escape($value['name']);
					elseif( isset($dbinfo['upload_dir']) && is_file($dbinfo['upload_dir'] . "/" . db_escape($value['name'])) )
						$del_uploadfile[] = $dbinfo['upload_dir'] . "/" . db_escape($value['name']);
				} // end if
			} // end foreach
...
function modify_bank_ok(){
	...
	// 수정 권한 체크와 해당 게시물 읽어오기
	if(privAuth($dbinfo,"priv_delete")) // 게시판 전체 삭제 권한을 가졌다면 수정 권한 무조건 부여
		$rs_modify=db_query("SELECT * FROM {$table_bank} WHERE uid='" . db_escape($qs['uid']) . "'");
	elseif(isset($_SESSION['seUid'])) // 회원의 글이라면,
		$rs_modify=db_query("SELECT * FROM {$table_bank} WHERE uid='" . db_escape($qs['uid']) . "' and bid='" . db_escape($_SESSION['seUid']) . "'");
	else { // 비회원의 글이라면 (비회원의 글에 패스워드가 없을 경우 누구든지 수정 가능, 실수로 안 입력했을 경우 수정가능하게)
		$rs_modify=db_query("SELECT * FROM {$table_bank} WHERE uid='" . db_escape($qs['uid']) . "' and passwd=PASSWORD('" . db_escape($qs['passwd'] ?? '') . "')");
	} // end if
	$list = db_count() ? db_array($rs_modify) : back("게시물이 없거나 수정할 권한이 없습니다");
	...
	if( $cateuid <> ($list['cateuid'] ?? '') ){
		$type = $list['type'] ?? '';
		$num = $list['num'] ?? 0;
		db_query("update {$table_bank} set cateuid='" . db_escape($cateuid) . "' where type='" . db_escape($type) . "' and num='" . db_escape($num) . "'");
	} // end if
...
function delete_bank_ok(){
	...
	// 삭제 권한 체크와 해당 게시물 읽어오기
	if(privAuth($dbinfo,"priv_delete")) // 게시판 전체 삭제 권한을 가졌다면
		$rs_delete=db_query("SELECT * FROM {$table_bank} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "'");
	elseif(isset($_SESSION['seUid'])) // 회원의 글이라면,
		$rs_delete=db_query("SELECT * FROM {$table_bank} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "' and bid='" . db_escape($_SESSION['seUid']) . "'");
	else { // 비회원의 글이라면
		if(isset($qs['passwd']) && $qs['passwd'])
			$rs_delete=db_query("SELECT * FROM {$table_bank} WHERE $sql_where and uid='" . db_escape($qs['uid']) . "' and passwd=PASSWORD('" . db_escape($qs['passwd']) . "')");
		else
			go_url("./delete.php?" . href_qs("uid=" . db_escape($qs['uid']),$qs_basic),0,"비밀번호를 입력하십시오"); // 비밀번호 입력 페이지로 이동
	} // end if
	...
	$rs_subre = db_query("SELECT * FROM {$table_bank} WHERE $sql_where and num='" . db_escape($list['num']) . "' AND LENGTH(re) > LENGTH('" . db_escape($list['re']) . "') AND LOCATE('" . db_escape($list['re']) . "',re) = 1");
	while($row=db_array($rs_subre)){
		if(isset($row['upfiles'])){
			$upfiles=unserialize($row['upfiles']);
			if(!is_array($upfiles)) { // 시리얼화된 변수가 아닌 파일 명으로 되어 있을 경우
	...
	db_query("DELETE FROM {$table_bank} WHERE $sql_where and num='" . db_escape($list['num']) . "' AND LENGTH(re) > LENGTH('" . db_escape($list['re']) . "') AND LOCATE('" . db_escape($list['re']) . "',re) = 1");
	db_query("DELETE FROM {$table_bank} where $sql_where and uid='" . db_escape($list['uid']) . "'");
...
function sysconfig_ok(){
	...
	$sql_set=", `site_name`		= '" . db_escape($site_name) . "',
				`homepage`		= '" . db_escape($homepage) . "',
				`c_name`		= '" . db_escape($c_name) . "',
				`ceo_name`		= '" . db_escape($ceo_name) . "',
				`b_conditions`		= '" . db_escape($b_conditions) . "',
				`b_item`		= '" . db_escape($b_item) . "',
				`c_tel1`		= '" . db_escape($c_tel1) . "',
				`c_tel2`		= '" . db_escape($c_tel2) . "',
				`c_fax`		= '" . db_escape($c_fax) . "',
				`c_email`		= '" . db_escape($c_email) . "',
				`c_biznum`		= '" . db_escape($c_biznum) . "',
				`c_zipcode`		= '" . db_escape($c_zipcode) . "',
				`c_addr`		= '" . db_escape($c_addr) . "'
			";

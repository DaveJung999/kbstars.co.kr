...
//===================
// 서치 게시물만..
$sql_where = isset($sql_where) ? $sql_where : " 1 ";
...
$rs_today = db_query("SELECT count(*) FROM {$table_popup} WHERE (rdate > unix_timestamp(curdate())) and  $sql_where ");
$count['today']=db_count($rs_today);
...
if ($modify_uid != "" ){
	...
	if (db_count() > 0){
		...
		if(!isset($list_modify['skin']) || !$list_modify['skin']) $list_modify['skin'] = "default";

...
//=======================================================
	...
	while( $list=db_array($result) ) {
		...
		$outCal[$list['startdate']][$tmp_hour] .= "<img src=images/micon.gif border=0><font face=굴림><span style='font-size:9pt'><a href='{$href['view']}' onMouseOver=\"view('{$list['title']}', '{$lhour}','{$list['content']}');\" onMouseOut=\"noview();\">{$list['title']}</a></span></font><br> \n"	;
	} // end while
	////////////////////////////
	...
	while( $list=db_array($result) ) {
		...
		while($tmp_time<=$tmp_time_enddate) {// 말일을 지나기 전까지
			...
			$outCal[$tmp][$tmp_hour] .= "<img src=images/micon.gif border=0><font face=굴림><span style='font-size:9pt'><a href='{$href['view']}' onMouseOver=\"view('{$list['title']}', '{$lhour}','{$list['content']}');\" onMouseOut=\"noview();\">{$list['title']}</a></span></font><br> \n"	;
...
<table align=center border="0" width="590" cellspacing="0" cellpadding="0" bordercolor="000000" bordercolorlight="000000">
<tr>
<td>
...
for  ($i=7 ;$i <= 20; $i++) {
	...
	if(($i%2) == 0 ) {
		...
		echo "	<td align=right width=70 height=22 bgcolor=c7c7c7> \n";
	}
	else {
		echo "<tr> \n";	
		echo "	<td align=right width=70 height=22 bgcolor=d7d7d7> \n"	 ;
	}
	...
	if($enable_write) 
		echo "<font face=굴림><a href='{$href['goinput']}'><span style='font-size:9pt'>{$i} : 00 </span><img src=images/add.gif border=0></a></font> &nbsp; \n";
	else 
		echo "<font face=굴림><span style='font-size:9pt'>{$i} : 00 </span></font> &nbsp; \n";
	echo "	</td> \n";
	echo "	<td align=left > \n";
...
// URL Link
$href['intPrevDate'] = "{$_SERVER['PHP_SELF']}?" 
					. href_qs("mode=day&date={$intPrevDate}",$qs_basic);
$href['intNextDate'] = "{$_SERVER['PHP_SELF']}?" 
					. href_qs("mode=day&date={$intNextDate}",$qs_basic);

	echo "<a href='{$href['intPrevDate']}'>";	
	echo "◀ 이전일[{$intPrevDate}] </a>&nbsp;&nbsp;";
	echo "<a href='{$href['intNextDate']}'>"	;
	echo " 다음일 [{$intNextDate}] ▶ </a>";
?>
</span></p>

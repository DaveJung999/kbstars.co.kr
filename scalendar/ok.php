...
//=======================================================
	...
	if(isset($_REQUEST['db'])) {
		$sql = "SELECT * FROM {$table_calendarinfo} WHERE db='" . db_escape($_REQUEST['db']) . "'";
...
function write_ok($table,$qs)
{
	...
	// 넘어온값 체크
	$qs=check_value($qs);
	if( !preg_match("/^[0-9]{4}-[01]?[0-9]-[0-3]?[0-9]$/",$qs['startdate']) ) {
		back("시작날짜가 잘못되었습니다");
	}
	if($qs['retimes']>0) {// 반복설정이면
		$qs['startdate'] = date("Y-m-d",strtotime($qs['startdate']));
		if( !preg_match("/^[0-9]{4}-[01]?[0-9]-[0-3]?[0-9]$/",$qs['enddate']) ) {
			back("반복날짜가 잘못되었니다");
		}
	...
	elseif ($qs['dtype'] == "month" )	{
		$qs['startdate']	= substr($qs['startdate'],0,8) . "01";
		$qs['starthour']	= "00";
		$qs['startmin']	= "00";
		$qs['endhour']	= "23";
		$qs['endmin']		= "59";
...
function delete_ok($table,$field="uid")
{
	...
	// 권한체크
	if(!privAuth($dbinfo, "priv_delete")) {
		if(($list['bid'] ?? 0) != ($_SESSION['seUid'] ?? 0)) back("삭제 권한이 없습니다");
	}

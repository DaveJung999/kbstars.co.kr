...
/*	if($_SERVER['REMOTE_ADDR'] == '210.104.250.41'){
		print_r($_SESSION);
		echo "333<br>";
	}*/
		
	// 로그인 후 이동할 페이지를 결정합니다.
	// $_REQUEST['goto']가 비어있지 않으면(null이나 빈 문자열이 아니면) 사용
	// 그렇지 않으면 $_SESSION['seREQUEST_URI']를 사용
	// 둘 다 비어있으면 '/'를 사용
	$goto = isset($_REQUEST['goto']) ? $_REQUEST['goto'] : (isset($_SESSION['seREQUEST_URI']) ? $_SESSION['seREQUEST_URI'] : '/');
	...
	/////////
	// 로그인
	if(isset($_POST['mode']) && $_POST['mode'] == 'login' && isset($_POST['userid']) && isset($_POST['passwd'])){
		// 0.1초간 쉬자. . 로그인에 0.1초 기다리게 할 수 있겠지.
		usleep(100000); // 패스워드 무한루트 돌리는 해킹을 조금이라도 무력화 시키기 위해
		...
		if( !isset($_SESSION['seHTTP_REFERER']) && isset($_SERVER['HTTP_REFERER'])
			 && strpos($_SERVER['HTTP_REFERER'],$_SERVER['HTTP_HOST']) === false ){
			$_SESSION['seHTTP_REFERER']	= $_SERVER['HTTP_REFERER'];
		}
		$remote_addr = $_SERVER['REMOTE_ADDR'];
		$ref_uri = isset($_SESSION['seHTTP_REFERER']) ? db_escape($_SESSION['seHTTP_REFERER']) : '';
		
		$sql = "INSERT INTO {$table_log_wtmp} SET bid='{$list['uid']}', ip='{$remote_addr}', ref_uri='{$ref_uri}', rdate=UNIX_TIMESTAMP()";
		db_query($sql);
		
		$sql = "DELETE FROM {$table_log_lastlog} WHERE bid='{$list['uid']}' AND gid='0'";		
		db_query($sql);
		
		$sql = "INSERT INTO {$table_log_lastlog} SET bid='{$list['uid']}', gid='0', port='www', ip='{$remote_addr}', ref_uri='{$ref_uri}', rdate=UNIX_TIMESTAMP()";
		db_query($sql);
		...
		// 운영자 출석체크
		/*if(isset($_SESSION['sePriv']['운영자']) and is_file('../sadmin/intranet/check_attendance.php')){
		...
		/*if ( strpos($goto, "https://") === true )
			...
			$goto = "http://www.kbstars.co.kr".$goto;*/
/*	if($_SERVER['REMOTE_ADDR'] == '210.104.250.41'){
		print_r($_SESSION);
		exit;
	}*/
		$Action_domain = $Action_domain ?? '';
		$goto = filter_var($goto, FILTER_SANITIZE_URL);
		
		go_url($Action_domain.$goto);
		exit;
	} // end if
	...
	if($dbinfo['enable_getinfo'] == 'Y'){
		...
		if( isset($_GET['html_skin']) && preg_match('/^[_a-z0-9]+$/',$_GET['html_skin'])
			and is_file($SITE['html_path'].'index_'.$_GET['html_skin'].'.php') )
			$dbinfo['html_skin'] = $_GET['html_skin'];
		if( isset($_GET['skin']) && preg_match('/^[_a-z0-9]+$/',$_GET['skin'])
...
//davej............추가.............
if (!isset($_REQUEST['goto']) && isset($_SESSION['seREQUEST_URI'])) $_REQUEST['goto'] = $_SESSION['seREQUEST_URI'];

$_REQUEST['goto'] = htmlspecialchars($_REQUEST['goto'],ENT_QUOTES);
$form_loginpage=" method='post' action='{$Action_domain}{$_SERVER['PHP_SELF']}'>
				<input type='hidden' name='goto' value='{$_REQUEST['goto']}'>
				<input type='hidden' name='mode' value='login'";
$form_loginguest=" method='post' action='{$Action_domain}{$_SERVER['PHP_SELF']}'>
				<input type='hidden' name='goto' value='{$_REQUEST['goto']}'>
				<input type='hidden' name='mode' value='loginguest'";
$form_remotelogin=" method='post' action='{$Action_domain}{$_SERVER['PHP_SELF']}'>
				<input type='hidden' name='goto' value='{$_REQUEST['goto']}'>

...
//=======================================================
	...
	// 비밀번호가 두번 정확했는지...
	if($_POST['passwd'] != $_POST['passwd_ok']) back('비밀번호를 두번 정확히 입력바랍니다.');
	...
	case '회원' :
		...
		if(date('y')+100-$tempage < 14){
			$qs=array_merge($qs,array( 
					...
					'p_idnum' =>  'post,trim,notnull='	. urlencode('부모님 주민번호를 정확히 입력하시기 바랍니다.')
			));
			$check_ok = true;
...
// 값 체크 마무리
if(!preg_match("/^[a-z][a-z0-9]+$/i", $qs['userid'])) // Userid 체크 (eregi -> preg_match)
	back('아이디는 5-10자까지 숫자, 영문자의 조합만 가능합니다 . 첫문자는 영문자여야 합니다.');
...
if($qs['priv'] == '회원') { // 개인의 경우 주민번호체크와 주민번호, 우편변호 값 설정
	// DB에 등록된 주민번호인지 체크
	$sql = "SELECT * from {$table_logon} 
			WHERE name='{$qs['name']}' 
			and email='{$qs['email']}' 
			and concat(substring(birth, 6, 4), substring(birth, 1, 4)) ='{$_POST['birth1']}{$_POST['birth2']}{$_POST['birth3']}' ";
	if(db_count(db_query($sql)))
		back('이미 등록 된 사용자입니다.\\n등록 한 계정으로 로그인 하여 사용 가능합니다.\\n혹, 계정정보가 생각이 나지 않으시면 아이디 및 비밀번호 찾기로 계정정보를 찾으실 수 있습니다.');	
}
...
/////////////////////////////////
// 추가되어 있는 테이블 필드 포함
$skip_fields = array( 'uid', 'rdate');
if($fieldlist = userGetAppendFields($table_logon, $skip_fields)){
	foreach($fieldlist as $value){
		...
		switch($value){
			...
			case 'zip' :
				...
				break;
			case 'address' :
				$qs['address'] = $_POST['address1']." ".$_POST['address2'];
				break;
			case 'tel' :
			...
			case 'hp' :
				...
				break;
			case 'nickname' :
				$qs['nickname'] = $_POST['name'] ;
				break;
			case 'yesmail' :
				$qs['yesmail'] =	$_POST['yesmail']? $_POST['yesmail'] : '0';
				break;
			case 'yessms' :
				$qs['yessms'] =	$_POST['yessms']? $_POST['yessms'] : '0';
				break;
			case 'host' :
...
//sfmail_join('2015_d12',$qs['email'],$qs['name']);
...
##	가입 환영 페이지로 이동
go_url("{$Action_domain}/sjoin/jointhankyou.php");
//back('회원가입이 완료되었습니다.\\n메인페이지로 이동합니다');
//back('회원가입이 완료되었습니다.\\n메인페이지로 이동합니다','/');
//=======================================================

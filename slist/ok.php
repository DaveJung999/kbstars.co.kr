...
//=======================================================
	function write_ok(&$dbinfo, $qs){
		global $db_conn; // 전역변수로 db_conn 추가
		$sql_where = ' 1 '; // init
...
// slist write
					...
					case 'num' :
						...
						break;
					case 'bid' :
						$qs['bid']	= isset($_SESSION['seUid']) ? $_SESSION['seUid'] : 0; // $_SESSION['seUid']가 없으면 0으로 초기화
						break;
					case 'userid' :
...
// sql_set 만듦
				if(isset($qs[$value])){
					if($value == 'passwd') $sql_set .= ", passwd = PASSWORD('".$qs['passwd']."') "; // PASSWORD() 함수로 변경
					else $sql_set .= ", {$value} = '".db_escape($qs[$value]) . "'"; // db_escape 적용
...
// 해당 게시물 읽어오기
		$sql = "SELECT *, PASSWORD('".db_escape($_POST['passwd']) . "') as pass FROM {$dbinfo['table']} WHERE uid='".db_escape($qs['uid']) . "' and $sql_where LIMIT 1";

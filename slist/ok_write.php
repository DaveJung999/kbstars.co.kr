...
switch($_REQUEST['mode'] ?? ''){
	case 'write':
		...
		// 어느 페이지로 이동할 것인지 결정
		$goto = $_REQUEST['goto'] ?? ($dbinfo['goto_write'] ?: 'read.php?' . href_qs('uid='.$uid,$qs_basic));
		back('',$goto);
		break;
	case 'modify':
		modify_ok($dbinfo, $qs);
		...
		// 어느 페이지로 이동할 것인지 결정
		$goto = $_REQUEST['goto'] ?? ($dbinfo['goto_modify'] ?: 'read.php?' . href_qs('uid='.($_REQUEST['uid'] ?? ''),$qs_basic));
		back('',$goto);
		break;
	case 'delete':
		delete_ok($dbinfo);
		...
		// 어느 페이지로 이동할 것인지 결정
		$goto = $_REQUEST['goto'] ?? ($dbinfo['goto_delete'] ?: 'list.php?'.href_qs('uid=',$qs_basic));
		back('',$goto);
		break;
	default :
		back('잘못된 요청입니다.');
...
function modify_ok(&$dbinfo,$qs){
	...
	$sql = "SELECT *,PASSWORD('{$passwd_safe}') as pass FROM {$dbinfo['table']} WHERE uid='".db_escape($qs['uid']) . "' and $sql_where LIMIT 1";
	...
	if(!privAuth($dbinfo,'priv_modify') ){
		if(isset($list['bid']) && $list['bid']>0){
			...
			if( isset($list['passwd']) && isset($list['pass']) && $list['passwd'] != $list['pass']) back('정확한 비밀번호를 입력하여 주십시오');
		}
	} // end if
...
function delete_ok(&$dbinfo){
	...
	if(!privAuth($dbinfo,'priv_delete')) {// 게시판 전체 삭제 권한을 가졌다면
		...
		elseif(isset($list['bid']) && $list['bid']>0) { // 회원이면
			...
			if(isset($list['passwd']) && isset($list['pass']) && $list['passwd'] != $list['pass']){

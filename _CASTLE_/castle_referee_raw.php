...
function castle_referee_alert($msg){
	...
	*/
	...
	if ($policy['alert_alert'] == "TRUE"){
		...
		// iconv가 실패할 경우를 대비하여 @ 추가
		$alert_msg_euckr = @iconv("UTF-8", "UTF-8", $alert_msg_utf8);

		$alert_msg_js	= "<script>\n"; // language="javascript" 속성은 HTML5에서 불필요
		...
		$alert_msg_js .= "\n";
		$alert_msg_js .= "	if (CastleBrowser.toLowerCase() == \"utf-8\") \n"; // 대소문자 구분 없도록 toLowerCase() 추가
		$alert_msg_js .= "		alert('" . addslashes($alert_msg_utf8) . "');\n"; // addslashes 추가
		$alert_msg_js .= "	else \n";
		$alert_msg_js .= "		alert('" . addslashes($alert_msg_euckr) . "');\n"; // addslashes 추가
		$alert_msg_js .= "\n";
...
function castle_referee_eregi($regexp, $str){
	...
	$str = castle_referee_htmldecode($str);
		...
		// slashes 덧붙임 제거
	// get_magic_quotes_gpc()는 PHP 7.4에서 제거되었으며 항상 false를 반환합니다.
	...
	// 정규표현식 체크
	// eregi()는 제거되었으므로 preg_match()와 i(대소문자 무시) 플래그 사용
	
	$esc_regexp = str_replace("/", "\/", $regexp);
		/* 서버 설정이 eucKR인 경우 */
	// iconv가 실패할 경우를 대비하여 @ 추가
	if (@iconv("UTF-8", "UTF-8", $str) && preg_match("/" . $esc_regexp . "/i", iconv("UTF-8", "UTF-8", $str))) 
		return $regexp;
...
function castle_referee_unhtmlentities($string){
	// preg_replace의 /e 수정자를 preg_replace_callback으로 변경
	...
	// get_html_translation_table은 여전히 유효함
...
function castle_referee_htmldecode($str){
	...
	$callback_dec = function($matches){
		return chr($matches[1]);
	};

		// Hex encoding '&#x20;' -> ' '
	while (preg_match('~&#x([0-9a-f]+);~i', $str))
		$str = preg_replace_callback('~&#x([0-9a-f]+);~i', $callback, $str);
...
function castle_referee_delete_special_characters($str){
	$str = str_replace(chr(0), '', $str); // Null byte
	$str = str_replace(chr(10), '', $str); // Line feed
...
function castle_referee_error_handler($policy_type, $rule, $method, $key, $value, $msg = ""){
	...
	if ($policy['log_simple'] == "TRUE"){
		...
		// 문자열 인코딩 감지 로직 단순화 (mb_detect_encoding 사용 권장)
		// 기존 로직을 유지하되, @iconv로 오류 억제
		$is_euckr = @iconv('UTF-8', 'UTF-8', $value) === false;

		if ($is_euckr){
			if ($policy['log_charset']['UTF-8'] == "TRUE"){
				$log['simple'] .= @iconv("UTF-8", "UTF-8", $_SERVER['PHP_SELF'] . ": ");
				$log['simple'] .= @iconv("UTF-8", "UTF-8", $key	. " = " . mb_substr($value, 0, 64, 'UTF-8') . ": ");
				$log['simple'] .= $msg	. "\n";
			} else {
				$log['simple'] .= $_SERVER['PHP_SELF'] . ": ";
				$log['simple'] .= $key	. " = " . mb_substr($value, 0, 64, 'UTF-8') . ": ";
				$log['simple'] .= @iconv("UTF-8", "UTF-8", $msg	. "\n");
			}
		} else {
			/* 서버 설정이 UTF-8인 경우 */
			if ($policy['log_charset']['UTF-8'] == "TRUE"){
	...
	if ($policy['log_detail'] == "TRUE"){
		...
		if ($is_euckr){
			if ($policy['log_charset']['UTF-8'] == "TRUE"){
				$log['detail'] .= @iconv("UTF-8", "UTF-8", $_SERVER['PHP_SELF'] . ": ");
				$log['detail'] .= @iconv("UTF-8", "UTF-8", $key	. " = " . mb_substr($value, 0, 64, 'UTF-8') . ": ");
				$log['detail'] .= $msg	. "\n";
			} else {
				$log['detail'] .= $_SERVER['PHP_SELF'] . ": ";
				$log['detail'] .= $key	. " = " . mb_substr($value, 0, 64, 'UTF-8') . ": ";
				$log['detail'] .= @iconv("UTF-8", "UTF-8", $msg	. "\n");
			}
		} else {
			/* 서버 설정이 UTF-8인 경우 */
			if ($policy['log_charset']['UTF-8'] == "TRUE"){
		...
		if ($policy['log_charset']['UTF-8'] == "TRUE"){
			...
			$log['detail'] .= @iconv("UTF-8", "UTF-8", " -> [Method: " . $method	. "]\n");
			$log['detail'] .= @iconv("UTF-8", "UTF-8", " -> [Policy: " . $policy_type	. "]\n"); 
			$log['detail'] .= @iconv("UTF-8", "UTF-8", " -> [Pattern: " . $rule	. "]\n");
		}
...
function castle_referee_detect_sql_injection($string){
	...
	foreach ($policy['sql_injection_list'] as $regexp)
	{
		...
		$detected_pattern = castle_referee_eregi($regexp, $string);
		if ($detected_pattern)
...
function castle_referee_check_file_policy(){
	global $_CASTLE_POLICY, $_FILES; // $_FILE -> $_FILES 로 수정
	...
	{
		// 업로드된 파일이 여러개인 경우를 대비
		if (is_array($fileinfo['name'])){
			// 이 부분은 다중 파일 업로드에 대한 추가적인 처리가 필요할 수 있습니다.
			// 현재는 첫 번째 파일만 검사합니다.
			$fileinfo = array(
		...
		$check['filevar'] = $var;
		$check['filetype'] = pathinfo($fileinfo['name'], PATHINFO_EXTENSION);
		$check['filesize'] = $fileinfo['size'];

			// 파일 업로드 체크
		if ($fileinfo['error'] != UPLOAD_ERR_OK)
			continue; // continue로 변경하여 다른 파일 검사를 계속 진행
...
function castle_referee_check_basic_policy(){
	global $_CASTLE_POLICY, $_CASTLE_POST_DATA, $_SERVER;

		// 페이지 이름을 가져옴
	$check['page_name'] = $_SERVER["SCRIPT_NAME"];
	...
	if (isset($_CASTLE_POLICY['CONFIG']['TARGET']['POST'])){
		...
		if ($policy['post_bool'] == "TRUE"){
			// Raw POST Data 인 경우
			// $HTTP_RAW_POST_DATA는 PHP 7에서 제거됨. file_get_contents('php://input') 사용
			$raw_post_data = file_get_contents('php://input');
			if (isset($raw_post_data)){
				$xml_data = $raw_post_data;
				...
				xml_parser_free($xml_parser); // 리소스 해제
			} else {
				// Typical POST인 경우
	...
	if (isset($_CASTLE_POLICY['CONFIG']['TARGET']['GET'])){
		...
		if ($policy['get_bool'] == "TRUE"){
			foreach ($_GET as $key =>	$value) 
			{
				...
				if ($policy['xss'] == "TRUE"){
					$pattern = castle_referee_detect_xss($value); // 함수 호출 수정
					if ($pattern)
	...
	if (isset($_CASTLE_POLICY['CONFIG']['TARGET']['POST'])){
		...
		if ($policy['post_bool'] == "TRUE"){
			foreach ($_POST as $key =>	$value) 
			{
				...
				/* 변수 데이터 형태별 정책 적용 */
				$policy['format'] = "";
				if (isset($policy['var']['FORMAT'])) // isset으로 확인
					$policy['format'] = $policy['var']['FORMAT'];

...
/* 전역변수 호환성 설정 */
	// PHP 7은 'register_globals'를 지원하지 않으므로, 전역 변수를 명시적으로 선언합니다.
	if (isset($_REQUEST)){
		$submit = $_REQUEST;
	}
...
\n\n");
		...
		foreach ($array as $key =>	$value)
		{
			...
			if (!is_array($array[$key]))
				fwrite($fd, "\t\$_CASTLE_POLICY['".$key."'] = \"".$value."\";\n");
	...
	{
		global $page, $print, $submit, $policy;
	...
	function castle_check_authorized()
	{
		...
		$auth['remote_addr'] = $_SERVER["REMOTE_ADDR"];
